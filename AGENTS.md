...
- **`as const` for static config objects**: When you do define a module-level const object (style object, dispatch table, options array, etc.), end it with `as const` rather than annotating it with `: CSSProperties` / `: Record<…, …>` / etc. Example: `const BAR_STYLE = { display: "flex", … } as const;`. This gives literal types to the properties, prevents accidental mutation, and avoids importing the type-annotation just for one declaration.
