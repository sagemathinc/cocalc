...
import datetime, socket, time   # system libraries
...
class BackendConnectionServer(object):
    ...
    def accept_backend_connection(self, fd, events):
        while True:
            try:
                connection, address = backend_socket.accept()
    ...
    def handle_backend_connection(self, connection, address):
        class C(object):
            ...
            def idle_check(self):
                ...
                if time.time() - C.last_mesg_time >= self._idle_timeout:
                    C.stream.close()
                else:
                    iol.add_timeout(datetime.timedelta(seconds=float(self._idle_timeout)), c.idle_check)
        c = C()
        c.idle_check()
        get_next_message()
