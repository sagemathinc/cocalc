...
- [ ] open project (somewhere) -- should immediately refuse to even *try* to mount if `pid zpool` is nonempty (or has an error code)
      this indicates a machine is booting and then scanning the pool which takes a long time...

- [ ] (2:00?) move code for close idle projects to hub, done via a database query.
- [ ] (1:30?) user crontabs
