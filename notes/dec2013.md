...
- [ ] (1:30?) user crontabs
- [ ] (1:00?) open project (somewhere) -- should immediately refuse to even *try* to mount if `pid zpool` is nonempty (or has an error code) this indicates a machine is booting and then scanning the pool which takes a long time...
