# Replication of project data

finish basic storage and replication:

-->  - [ ] (0:30?) add hashring to storage.py: python implementation sucks... so I had to rewrite it to have same features as node.js one
...
r = new HashRing({'10.1.1.4':{vnodes:256}, '10.1.2.4':{vnodes:128}, '10.1.3.4':{vnodes:128}}, 'md5', {replicas:2});0;

  - [ ] (0:30?) create topology file and push out to /home/storage/topology
  - [ ] (0:30?) send with no inputs: directly send to all replicas at all datacenters (I can do the gossip approach later)
  - [ ] (0:30?) run send on lots of projects and see how it goes.
  - [ ] (1:00?) make a daemon started from admin.py (which pushes out the storage/topology file), and monitors zpool history and sends each project with newly *created* snapshots on this node (created by the zfs snapshot command).
  - [ ] (1:00?) make the daemon periodically snapshot active projects (maybe use inotify or zfs diff or something...)

Once the above is done and working, make it replicate out, set up the google DC and start replicating there too.
And while that is running, work on adapting the hub to use this stuff.
