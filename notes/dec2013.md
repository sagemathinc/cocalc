...
- [ ] (1:00?) when in .zfs/snapshot/ path, include a restore button.
- [ ] (2:00?) restore file from snapshot (better copy)
- [ ] (1:30?) display snapshots in gui in local user time
- [ ] (1:30?) user crontabs: where do they live?  where do they run?
- [ ] (2:00?) ipython notebook proxying seems maybe broken by changes to hub.
- [ ] (1:30?) make info.json get written by the create_project_user.py script (and not by hub).  All info in info.json is trivial to obtain in the new model.  With moving, it is actually *WRONG* information, since the host changes (of course hub fixes this).
- [ ] (2:00?) implement migrate again in storage.coffee, so that it can interact with database and be much more parallel.
