...
Google:
 ...
 - [ ] apt-get install strace iotop host
 - [ ] sudo vi /etc/ssh/sshd_config; sudo /etc/init.d/ssh restart  # allow passwords; also MaxStartups 120
 - [ ] make tinc conf's permanent
 - [ ] sudo chmod +x /bin/fusermount; sudo chmod a+rw /dev/fuse
 - [ ] create /usr/local/bin/ipython script.
 - [ ] in salvus do this (and add this to build.py!?):
         cd salvus/salvus/
         mkdir local_hub_template/node_modules
         cp scripts/skel/.sagemathcloud/node_modules/*.js local_hub_template/node_modules/
         ./make_coffee --all
- [ ] sudo cp ~/salvus/salvus/scripts/create_project_user.py /usr/local/bin/


# Storage2

- [ ] (1:00?) re-implement the snapshot "timeago" thing.
- [ ] (1:00?) when in .zfs/snapshot/ path, include a restore button (that pops up a dialog)
- [ ] (1:00?) update base vm's
- [ ] (1:30?) create new project -- verify it works.
