...
-->  - [ ] global synchronization -- no matter what, we need functionality that computes the latest snapshots name for all projects... it's built into ZFS!
...
-->  - [ ] storage: create a quota command, which increased the quota on *all* copies of the project; then test it on e89567b4-02f6-4947-9809-ac24e64ebbd8 on 10.1.1.4 (and any other big ones?), which is a MRC project.

  - [ ] storage: implement better work schedule that minimizes transfers between data centers; start this one running on another node.  test on 10.1.1.4 where the project e89567b4-02f6-4947-9809-ac24e64ebbd8 is HUGE and needs to get replicated to GCE.

  - [ ] write something to compute and run the strategy:


  - [ ] booting up host compute machines -- zfs doesn't work

  - [ ] booting up host compute machines -- no storage user.

  - [ ] we need to ensure the user is created
