...
- [x] (1:00?) open project -- should immediately refuse to even *try* to mount if `pid zpool` is nonempty (or has an error code) this indicates a machine is booting and then scanning the pool which takes a long time...  Alternatively, time out the zfs set mountpoint quickly (?). Accidental mounts left around don't really cause any trouble.

- [ ] (2:00?) finish migration...
- [ ] (1:00?) verify that number of snapshot is the same for all projects in all locations (delete old extra snapshot on location)
- [ ] (1:00?) spot check a bunch of projects to verify that project migration probably actually working...
- [ ] (1:30?) user crontabs: -- put something in .crontabs/ directory of project; install it when project starts.  remove when stops.
- [ ] (1:30?) add periodic call to close_stale_projects in hub; have projects that don't get this.
- [ ] SWITCH!

---

- [ ] better vm shutdown procedure -- so projects moves are smoother (?) -- e.g. -- a function I can call...

- [ ] delete the "modified_files" entry in the projects table everywhere; instead, we'll directly compute this information using the filesystem, which is very fast and accurate.
- [ ] delete projects from db that have no owner/location.
- [ ] (1:00?) UI: automatically update project location when it gets set.
- [ ] (0:30?) UI: make a link to snapshots in the settings where it says "snapshots"; similarly for "files".

- [ ] migrating this project may be HARD: 2c4292ec-6f42-4e79-9981-31f17c01443e and https://mail.google.com/mail/u/0/?shva=1#sent/14349f2966c28e0e


- [ ] (1:00?) remove the ssh equivalence from root on all compute nodes (or just reboot them all)
- [ ] (1:00?) when compute machine boots up it should NOT copy over /etc/passwd, etc. -- we don't want all those old accounts, since accounts are created on the fly.  also remove the copying over from create_project_user.py.
...
- [ ] (0:30?) change this in create_project_user.py after migration time:
...
- [ ] (3:00?) cassandra: spin up new DC at GCE
...
- [ ] how to backup offsite: just do "zfs send" to make a bunch of files `project_id-snapshot1-snapshot2.lz4.encrypt` and store those *anywhere*.
...
I'm re-migrating fc204d2c-1fbb-4726-b625-1ff1a45ef2f4 on 10.1.6.4 since it hits 10G and I did it before I had enabled dedup.  I'm curious what dedup does.  It is the only one where the quota error got hit.  Re-dedup made little difference... not sure how dedup really works behind the scenes.
  ...
  - [x] (0:45?) (2:27) storage2: create project --
   ...
   - [x] i just started a zpool scrub on 10.3.1.4 to see what happens.  It looks like it would take over a day per terabyte...
...
root@compute1dc2:/home/salvus# zpool status projects
 ...
 state: ONLINE
  scan: scrub in progress since Thu Dec 26 13:44:23 2013
    ...
    It finished with: "scrub repaired 0 in 7h44m with 0 errors on Thu Dec 26 21:29:22 2013"
   ...
   - [x]  tinc NETWORKING issue.
         I'm trying this on the two relevant nodes (it's what worked for the google networking):
             ifconfig tun0 mtu 1416
         FAIL at 8:25am

         Next try changing some tinc params -- this *should* be the default, but...
             LocalDiscovery = no
         FAIL at 9:17am
         However, restarting exactly one of the tinc ends (10.1.2.4) did make things work again instantly on both sides.
         ...
         Trying this script:
            ...
            while True:
                t = time.time()
                os.system('ping -c 1 -W 60 10.1.2.4')
                tm = time.time()-t
                if tm > 1:
                    open("BAD",'a').write("bad ping time of %s at %s"%(tm, time.strftime("%Y-%m-%d/%H:%M:%S")))
                    print "** BAD **\n"*5
                time.sleep(3)

         Trying this in **tinc.conf**, starting at about 10:50am.  Maybe re-keying is taking 30 seconds?
         Next time this happens, I need to check tincd in top!
            KeyExpire = 9999999999

         Nope, not KeyExpire.. at 11:15 had problem again.  Over 60 seconds!
         Try MTU params:

        Solved by Keith: fixed by doing "sudo ifconfig tun0 txqueuelen 10000" everywhere;
        also, this has improved the speed. 30MB/s and 50MB/s.

   - [x] (2:00?) hub: make responsive to project location from database.
        - local hub: change it to be defined by project id and *always* lookup location before (re-)connecting to local hub
        - on move, kill all processes, which forces reconnect -- only...
   - [x] (0:30?) make hub do open_project as step 1 of opening a local hub project (?)
         see this code in hub.coffee "# TODO: this is where we would deploy the project, right?"


- [x] (1:10) change *all* tinc daemons to re-key once per month: TCPonly=yes; Compression=10
- [x] (0:40) fix password reset
- [x] storage: diff function -- half done; "alter table projects add modified_files   map<text, text>;"
- [x] storage: new db table entry: last_snapshot:   "alter table projects add last_snapshot timestamp;"
- [x] hub: automate making snapshots
- [x] (0:17) hub: automate replicating
- [x] (0:57) hub: restart project server should: (1) force snapshot, (2) set location=undefined
- [x] (1:00?) option to the snapshot function to only snapshot if "interesting" files have changed.
- [x] hub: if current location "doesn't work", set location=undefined in database, so a new location gets chosen.
- [x] (0:45?) storage2 optimization: output of replicate (actually repair snapshot list) uses too much space.
- [x] (1:00?) (0:51) storage2: get usage in settings from df instead of du.

- [x] (1:00?) (1:37) improve how info.json is created -- raw file server in local_hub not working because of this line:  " raw_server.listen port, info.location.host, (err) ->"
     Basically the info object is wrong when the project gets moved.  I can fix this by just computing the info object; get rid
     of the stupid info.json file, which isn't needed anymore.
- [x] (1:00?) optimize hub proxy server; fix bug in editor when opening ipython, where it was timing out way too quickly.
- [x] (2:00?) (1:13) minimal awareness of read-only files: needed for browsing snapshots.
- [x] (2:00?) (3:10) make snapshots browse .zfs/snapshot when new-style project
    - PLAN 1:
        - clicking on the snapshot button simply loads ~/.snasphot/ as before
        - in project file browser, when the path is ~/.snapshot, we turn all first parts of path names into *local* times,
          then for each *unique* day display a directory link.  The URL stays as is.
        - in project file browser, when the path is ~/.snapshot/day/, compute the utc range for that day and get the paths
          in ~/.zfs/snapshot that fall in the given range.  Display them, but with nice looking local times.
        - in project file browser, when the path starts with ~/.zfs/snapshot/day/local_date[-tag], grab the listing by first converting
          local_date to UTC then getting the listing for ~/.snapshot/iso_utc_date/ and displaying it.
          Question: should the browser url be ~/.zfs/snapshot/iso_utc_date or ~/.snapshot/iso_utc_date?  I think the latter,
          so that it works across users.


- [x] (3:00?) (4:00) implement and start runing migrate again in storage.coffee, so that it can interact with database and be much more parallel.
...
- [x] Migration causes LOST PROJECTS, e.g.,
        select last_migrated,last_edited,location,owner,locations from projects where project_id=b062f3ca-f83b-40cc-a916-e3be80166ea7;
        # no location
        select * from last_snapshot where project_id=b062f3ca-f83b-40cc-a916-e3be80166ea7;
        # but recent snapshots
     Will recreate those later.  Must have been caused by "move" button being inadvertantly released.
     NO -- this is my script breaking things the first time.  UGH.
- [x] (1:00?) (3:00)  make migrate work when new (so doesn't whack location ever!!), then start running en mass.

- [x] (1:00?) write a reco letter to test things out!   https://mail.google.com/mail/u/0/?shva=1#search/letter/143356d1610d6074
...
Google:
 - [x] add lz4c: cd /tmp/; wget https://dl.dropboxusercontent.com/u/59565338/LZ4/lz4-r109.tar.gz; tar xvf lz4-r109.tar.gz; cd lz4-r109; make install; rm -rf /tmp/lz4-r109
 - [x] sage -sh; umask 022; pip install pyinotify
 - [x] add to /etc/sysctl.conf: # fs.inotify.max_user_watches=10000000
 - [x] apt-get install strace iotop host
 - [x] sudo vi /etc/ssh/sshd_config; sudo /etc/init.d/ssh restart  # allow passwords; also MaxStartups 120
 - [x] make tinc conf's permanent
 - [x] sudo chmod +x /bin/fusermount; sudo chmod a+rw /dev/fuse
 - [x] create /usr/local/bin/ipython script.
 - [x] in salvus do this (and add this to build.py!?):
         cd salvus/salvus/
         mkdir local_hub_template/node_modules
         cp scripts/skel/.sagemathcloud/node_modules/*.js local_hub_template/node_modules/
         ./make_coffee --all
- [x] sudo cp ~/salvus/salvus/scripts/create_project_user.py /usr/local/bin/


- [x] (1:00?) (1:00) update base vm's

MIGRATION:
  - keep close eye on space on cloud16 and cloud7!!!

---

- [x] (1:00?) (1:16) re-implement the snapshot "timeago" thing.
- [x] (1:30?) (2:00) storage2: create new project/ project restart
     - forces browser refresh
     - doesn't ever really create project
- [x] (1:00?) (0:39) project restart
- [x] (1:00?) (1:19) project move
- [x] (0:15?) (0:20) re-migrate 2c4292ec-6f42-4e79-9981-31f17c01443e
- [x] (1:00?) (1:36) when in .zfs/snapshot/ path
- [x] (2:00?) (0:46) database: write code to close idle projects
