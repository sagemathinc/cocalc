# Next update:
SMC:
 - [ ] add lz4c: sudo add-apt-repository ppa:gezakovacs/lz4; sudo apt-get update; sudo apt-get install liblz4-tool
...
I had to import on the command line this way, since piping in resulted in a timeout after 10 seconds:
  ...
  - [x] (0:30?) storage2: delete project from host

  - [ ] (0:45?) diff -- see files that have changed between snapshots or live and snapshot

  - [ ] (1:00?) database locking when snapshotting, so if two nodes try to do the same update, one gives error

  - [ ] (1:00?) storage2: repair -- implement version in storage2 and start running.
