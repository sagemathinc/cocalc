...
-->  - [ ] global synchronization -- no matter what, we need functionality that computes the latest snapshots name for all projects... it's built into ZFS!
  ...
  - [ ] storage: re-update/migrate a node and see how that goes.

  - [ ] storage: implement better work schedule that minimizes transfers between data centers; start this one running on another node.
