...
-->  - [ ] global synchronization -- no matter what, we need functionality that computes the latest snapshots name for all projects... it's built into ZFS!
  ...
  - [ ] storage: in replicate, (1) send one copy to each data center, then (2) tell each copy in the other data centers to send to the other node in that data center by ssh'ing over and calling storage.py.  This would take a little longer but will be more efficient network-wise.
