...
# Replication of project data -- High Availability "End Game"
  ...
  - [ ] (0:30?) (1:31 due to scope increase) storage2: get current usage and quota also store in database

            ALTER TABLE projects ADD usage_zfs varchar;
            ALTER TABLE projects ADD quota_zfs varchar;

-->  - [ ] (1:00?) migrate: amazingly the node.js script to do everything ran no problem to completion :-)
        However, there were 35 errors.  Look at them. They are in the file "y" on 10.3.1.3.

I'm re-migrating fc204d2c-1fbb-4726-b625-1ff1a45ef2f4 on 10.1.6.4 since it hits 10G and I did it before I had enabled dedup.  I'm curious what dedup does.  It is the only one where the quota error got hit.

  - [ ] (1:00?) by controlling the storage machines from the hubs, I can get rid of the ssh equivalence between storage nodes, which cuts the attack surface a lot...   I could just get rid of the storage account and visudo the salvus account to run "zfs".


 - [ ] (1:00?) storage2: create user
  ...
  - [ ] (2:00?) moving a project

  select location from projects where project_id=c1f1dc4a-dbf0-4fc6-9878-012020a0a829;

  update projects set location='{"host":"10.3.1.4","username":"c1f1dc4adbf04fc69878012020a0a829","port":22,"path":"."}' where project_id=c1f1dc4a-dbf0-4fc6-9878-012020a0a829;

   logged in and as root do

      import storage
      storage.create_user('c1f1dc4a-dbf0-4fc6-9878-012020a0a829')

   Also do:

       ssh storage@10.3.1.4 "./storage.py mount c1f1dc4a-dbf0-4fc6-9878-012020a0a829"
  ...
  - [ ] (1:00?) booting up host compute machines -- zfs doesn't work

  - [ ] (1:00?) booting up host compute machines -- no storage user.

  - [ ] (2:00?) allow user to manually make a snapshot with a tag; these never get deleted;

  - [ ] (1:30?) create /projects/root/etc and /projects/root/sys/crontabs and modify first_boot.py, and also create_unix_user.py
  ...
  - [ ] (0:30?) make hub mount project as step 1 of opening a local hub project (?)

  - [ ] (1:30?) hub: if location empty, compute hash to determine available locations, for each get time of last snapshot, and choose from amongst *newest* options at random

  - [ ] (1:00?) write code to do this (and run on test projects) --  for each project:
      - do a final sync and replicate
      - delete location in database

  - [ ] (2:00?) new snapshot ui -- basically just add a button to copy file back to original location for now, and warning message on open about read only.

  - [ ] (1:00?) delete several snapshot server disks to save space.

  - [ ] (2:00?) project move ui -- show info about available targets (location), last snapshot time, etc.   New message: "move project".
  ...
  - [ ] optimization: make sure to have 'zfs set snapdir=hidden' on everything.

  - [ ] change how usage is computed when a project is opened -- "du -sch ." just crashes with ZFS.
  ...
  - [ ] tuning dedup ram tradeoff: https://github.com/zfsonlinux/zfs/issues/1369
