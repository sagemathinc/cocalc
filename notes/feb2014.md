...
# Storage3:
 ...
 - [ ] I'm trying `zfs set sync=disabled projects; zfs set sync=disabled projects-new` on the pool on 10.1.2.4, to see if that helps
...
--> - [ ] storage3: figure out how to run on multiple machines without replicating work already done: create a query in database to get list of all project id's of projects that are on machine a, but not machines b,c,...
 ...
 - [ ] try again more cleverly to reconstruct the snapshot history by being more careful and copying exactly the changed files as root so permissions work out right...  We really want to maintain that history!
