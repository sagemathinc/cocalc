...
--> - [ ] (1:30?) (0:27+) login email address is case sensitive -- make it not case sensitive #95
         ...
         - fully test on my laptop vm install.
         - make database snapshot
         - fully dump backups of any tables we're going to modify: accounts and email_address_to_account_id
         - in the below, be careful about accounts that have same email up to case; search for those first?!
             - write code to go through email_address_to_account_id table and replace record by one with email_address.toLowerCase()
             - write code to go through accounts table and replace record by one with email_address.toLowerCase()
             - test on vm
         - now the scary part: push code to web servers; restart them; then run the above database update code.

- [ ] (3:00?) project move (dialog to provide info about targets).

- [ ] (0:20?) (sunday afternoon) verify encrypted copy of backup vm is copied to bsd, then restart locally.
