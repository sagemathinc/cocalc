# Next base image update:
   ...
   - [ ] sudo su; cd /usr/share/fonts/truetype; ln -s liberation ttf-liberation
   - [ ] sage -sh
         umask 022; pip install theano
   - [ ] install theano into new sage-6.1
...
--> - [ ] (4:00?) implement one basic cgroups controls on projects
  ...
  --> - [ ] (2:00?) when starting a project have option to create a new cgroup for that user (modify my create_project_user.py script)
...
- [ ] (0:45?) project move: gce status isn't available -- fix
...
- [ ] move -- somehow disable option to move to a machine that isn't available (according to db)
- [ ] (0:10?) swap out offsite usb disk.
