...
--> - [ ] (0:45?) project move: disable option to move to a machine that isn't available (according to db)

    grant select on table storage_topology to monitor;



- [ ] (0:45?) storage: change to just copy to all destinations simultaneously, since the zfs send/recv takes way, way longer than transfer, and this approach will thus take much less time overall.
...
- [ ] (0:30?) compute vm's: automate adding this line to storage crontab on startup:
