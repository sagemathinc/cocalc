- [ ] new release
     ...
     - [ ] much optimized use of cassandra
          - make sure to run update_... script once to make sure all accounts updated.
     - [ ] upgrade node
     - [ ] install certain node packages:
              npm install node-cassandra-cql
              git clone https://github.com/nodejitsu/node-http-proxy.git -b caronte; npm install node-http-proxy/


- Major projects:
    - scale up: make use of new hardware
...
# Goal: Rewrite cassandra schema and queries to:
    x - always use QUORUM consistency for ops...
    - Make sure all queries work without "allow filtering."
...
-- ARGH.  It turns out that collections don't work with Helenus... and there not working gets worse with the next few versions.
...
-->  - [ ] `project_users by account_id` -- or I could just add project collabs, etc. to the project itself and the account. Hmm.
        that feels dangerous.  What would happen though would:

 alter table projects add collaborators set<uuid>;
...
## Scans whole table -- be careful, esp. with QUORUM:
  - [ ] counting accounts, projects
...
# Insanely High priority issues
  ...
  - [ ] Massive bug: that filename/not matching directory path in UI thing leads to a huge bug where users will loose work, etc.
        see the log in bugs/
        the problem I think is that sessions are keyed by session id and filename, and filename is wrong...


- [ ] change database to not use index on email_address, etc., so can use consistency level quorum.
...
- [ ] upgrade to sage-5.12: http://boxen.math.washington.edu/home/release/sage-5.12/
