...
- We need a general message system for projects, i.e., a way for the hub to send general messages about a project to all connected clients that have those projects open...  Then there could be a message about sage server not working.  And much more.  But that's a very big project.  For now, what?

- For now, what makes the most sense is this: If you try to open a .sagews session, and it fails because the sage server isn't started, or won't start, just display an error saying so.    People who don't use sage will never be bothered, and people will get the message precisely when they need to.  OPTIMAL!

- [ ] restart local hub -- have it remove all cached codemirror sessions

- [ ] restart sage server -- just have it kill all sage_server.py processes

- [ ] command line open command -- I NEED IT! -- but how (?) -- seems impossible...

- [ ] restarting sage server / session is flaky.

- [ ] IPython flakiness issue -- it's related to 504 Gateway errors, which are either from the new node proxy server (in hub) or from haproxy -- just need to figure out which and change some timeout.  I get these also when refreshing my devel server.

     - simulate slow network: http://apple.stackexchange.com/questions/24066/how-to-simulate-slow-internet-connections-on-the-mac

     - [ ] replicate on storm?  not yet :-(
...
ALTER KEYSPACE "salvus" WITH REPLICATION = {'class' : 'NetworkTopologyStrategy', 'DCO':2, 'DC1':2}
