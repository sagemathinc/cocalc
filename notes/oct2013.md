...
- [ ] BUG -- missing icon for file editor on mobile!
...
what next - for maximal growth:

 - [ ] public read only browsing of projects:

Design ideaswhat next - for maximal growth:

 - [ ] public read only browsing of projects:



 Do *before* a new release that has the move functionality enabled that has the move functionality enabled that has the move functionality enabled:
...
EXPERIMENT (in devel project SMC in SMC):
 ...
 - [ ] hub -- when requesting url, parse and send message to client to open that thing.
