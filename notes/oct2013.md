...
- [ ] add some x's for clearing search boxes.
...
- [ ] (1:00?) sage server -- if broken, don't restart whole project server; instead provide a big warning.
- [ ] (0:30?) serving raw dot files: https://github.com/sagemath/cloud/issues/54
- [ ] (1:00?) fix ipython startup bug
- [ ] cache 10 new projects -- ready to go -- in the database.
- [ ] loading ipython notebooks first time is flakie/broken, maybe?



# Very High Priority

  - [ ] put the pre-made accounts in database, so any project server can use them: will make things more efficient (?)
