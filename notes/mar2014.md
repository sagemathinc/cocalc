...
- [x] rsync over vpn doesn't have to be encrypted -- much faster (?)

- [ ] client code: new way to get status of replicas

- [ ] client code: move snapshot browsing/listing to be client side in implementation

- [ ] - if is an abuse project (bitcoin) delete.
      - add in recent missing snapshots by pulling them remotely
      - get the absolutely latest live version of code as a new snapshot from any recently modified projects -- right before final switch
     >> - repack every bup repo.
      - delete bupindex -- it gets huge from migration, e.g., my project it was like 1.7GB being the dominant term!
      - run the few migrations that couldn't finish due to too many snapshots by just taking less snapshots.
