...
# TOP PRIORITY
 ...
 - [ ] rewrite hub so it uses new project system when project has a certain setting in db, testing on this new server...
    ...
    Plan:
     ...
     - [ ] add flag to projects table for "storage2 project" -- when set, fully use new storage system everywhere.
...
--> - [ ] storage -- refactored code to have some clean well-named operations, and also store their last usage in the database


--- FIRE: can't boot up vm's on 10.1.2.1!
...
This was missing:
 ...
 - [ ] disk space on cloud7 is getting problematic... 70G in "storage" so can reset that tomorrow morning.

 - [ ] do a query for which projects have nothing in the storage database yet, and migrate those, which should be solid and work...

 - [ ] storage_server: it could watch, make automatic snapshots, and push updates to the db, with *minimal-to-no involvement* from hubs (?).  Why should a hub do this stuff?
  ...
  - [ ]   { status: 'failed',
    project_id: '3a9b89b4-daea-49bb-a3d1-80e52e81e528',
    host: '10.1.17.4',
    error: 'No row in table \'storage\' matched condition \'[object Object]\'' },
