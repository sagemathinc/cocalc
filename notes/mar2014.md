...
- [ ] write a Python script `zfs_to_bup.py <project_id>` that fully converts a /projects/project-id that is mounted on the local machine to a directory project-id (in the path where run) that is a bup repo with all snapshots migrated, uid's fixed, etc.
...
- [ ] write script to make a bup repo with commit time the last snapshot time for each of the rsync'd projects above.

- [ ] completely delete cassandra database (again!) -- this time do replication factor of 3 everywhere -- the talks convinced me.

- [ ] read those bup repo's into the database.
