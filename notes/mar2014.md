...
- [ ] change `bup_storage.py` to work with zfs USE quotas (but keep traditional quota code):
...
- [ ] make `bup_storage.py` script much safer to run with sudo, e.g., remove options
- [ ] the `bup_server.coffee` script will run as salvus (not root)
- [ ] create `bup_storage.coffe` which provides api interface for bup-based storage to hub.
