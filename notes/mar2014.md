...
# TOP PRIORITY
  ...
  - [ ] storage/images/project-id fs keeps locking after one usage.  very disturbing.  WEIRD.  *must* address this.

  - [ ] storage: on successfully applying stream from database, delete anything in database that is redundant with that stream

  - [ ] rsync of the .sagemathcloud template
