...
new vm image:
   ...
   - [ ] mkdir -p /usr/local/sage/sage-6.2/local/share/sage/ext/octave/user; chmod a+rwx /usr/local/sage/sage-6.2/local/share/sage/ext/octave/user
   - [ ] update salvus repo
   - [ ] update visudo as smc_storage.py



# TOP PRIORITY

  - [ ] modify smc_storage to end by syncing over the current live version to the current live version, whether or not there is a snaphsot.  if any files are transferred, include that as a new snapshot with date now.

  - [ ] logging for storage

  - [ ] robustness of client when the remote server dies/moves to another port

  - [ ] start storage_servers running on all compute vm's.

  - [ ] for each project:
          - find out its current location from database
          - run migrate_delete there
          - record in database that we did an update at this point, since can't tell from streams. (so need table for this)
