- [ ] trying the following:
    - disable synchronizing the zfs calls -- i.e., raise that *_LIMIT param in zvol_storage.py
    - disable dedup everywhere, on zfs send, on the zpool's on zvols, and on the big storage zvol.
    - only try new projects -- clean, and if this works we'll have to start over with only things migrated after Sat Mar 22 04:36:55 UTC 2014
    - using daily ubuntu zfs build
    - QUESTIONS: - do we get kernel "120 second" locks?
                 - do we get kernel segfaults.
                 - make sure to remove dedup=on thing from compact function!
    - RESULT:
         - got *no segfaults* / locks at all.
         - got several "zvf create -V5G -s" that spun at 100% cpu, couldn't be killed, and went on forever.  They also blocked
           all future progress. Dang.    This was what I was getting with .img files before...

    - OK, now I'm going to try the same, but with the LIMIT set to 1 again, so no parallel ZFS stuff at all.
       RESULT:
          -

status=[];require('storage').migrate3_all(retry_errors:false,limit:50,only_new:true,status:status,cb:(e)->console.log("DONE",e))
status=[];require('storage').migrate3_all(oldest_first:true,retry_errors:false,limit:50,only_new:true,status:status,cb:(e)->console.log("DONE",e))


So far, so good.  I also tried renicing storage_server's node's stuff to -19, and that seems to be working well.  sudo renice -n -19 `pidof node` 
