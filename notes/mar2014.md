# Issues to remember
  ...
  - [x] whenever a save+sync happens successfully, store the last sync time in database

  - [ ] preparation:
          - switch the existing looping script to use RF=1
          - write code that goes through each vm (or project?) and
               - delets down to RF=1
               - bup restores files to working directory
            in preparation for going live tonight.

-->  - [ ] when deciding which host to start project on, order by last sync time
  ...
  - [ ] add bup quota as a standard part of settings, and refuse to make further snapshots if bup usage exceeds 3 times user disk quota.  This will avoid a horrible edge case.   Critical that this produces an error that the user learns about.  This will happen for some users.
  ...
  - [ ] consider again the possibility of not storing the live files longterm; only the bup repo. This is mainly to save space.    The key thing would be to include what is happening during restore in the status info for a project.  Also, replicating only the bup repos would be very, very fast and easy.

Some microbenchmarks in /tmp on cloud3 (a hard drive, not ramdisk):

Restoring shows "number of files restored", but doesn't know total number of files:

    salvus@cloud3:/bup/bups/3702601d-9fbc-4e4e-b7ab-c10a79e34d3b$ mkdir /tmp/x; time BUP_DIR=. bup restore master/latest --outdir=/tmp/x/
    Restoring: 97382, done.

    real    3m30.341s
    user    2m44.878s
    sys     0m15.949s
    salvus@cloud3:/bup/bups/3702601d-9fbc-4e4e-b7ab-c10a79e34d3b$ du -sch .
    727M    .
    727M    total

Time/space to tar.gz up the result:

    salvus@cloud3:/tmp/x$ time tar zcf latest.tar.gz latest
    tar: latest/salvus/salvus/debug: socket ignored

    real    1m59.107s
    user    1m39.794s
    sys     0m6.544s
    salvus@cloud3:/tmp/x$ ls -lht latest.tar.gz
    -rw-rw-r-- 1 salvus salvus 785M Apr  3 16:47 latest.tar.gz

Time to extract tarball:

    salvus@cloud3:/tmp/x$ time tar xf latest.tar.gz
    real    0m39.106s
    user    0m17.557s
    sys     0m9.097s

Time/space to use bup to create a single commit version of the above, and space used:

    salvus@cloud3:/tmp/x$ BUP_DIR=/tmp/x/bup bup init
    Initialized empty Git repository in /tmp/x/bup/
    salvus@cloud3:/tmp/x$ time BUP_DIR=/tmp/x/bup bup index latest/
    Indexing: 97382, done (5016 paths/s).
    real    0m19.790s
    user    0m17.421s
    sys     0m2.144s
    salvus@cloud3:/tmp/x$ time BUP_DIR=/tmp/x/bup bup save latest/ -n master
    Reading index: 97382, done.
    bloom: creating from 1 file (200000 objects).
    Saving: 100.00% (2965850/2965850k, 97382/97382 files), done.
    bloom: adding 1 file (60372 objects).
    real    1m52.385s
    user    1m36.970s
    sys     0m9.225s
    salvus@cloud3:/tmp/x$ du -sch bup
    708M    bup
    708M    total

Times are just as bad for bup restore on an SSD.   Conclusion -- just keep an extracted version.  Doubles the space, but oh well.  I can delete the .sagemathcloud, etc., directories periodically of unused projects, which would save a lot of space.
  ...
  - [ ] database schema update:
        ...
        alter table projects add bup_state     map<uuid, text>;
        alter table projects add bup_last_save map<uuid, timestamp>;

  - [x] should I switch to RF=1 per data center?  The argument for RF=2 only makes sense for something like cassandra, where clients only talk to one dc.  For this applicaition RF=1 would seem to be fine.  (?)  YES.

  - [x] when first starting a project its state keeps switching to stopped then starting then running - and also the start command gets executed 5 times... or so NOT GOOD
...
# Overall plan
 ...
 - [ ] get all projects migrated and copied out to their hosts
 - [ ] fix bugs/issues in new storage system running locally on my vm's
 - [ ] setup a testing hub that uses new system and new vm's and test
 - [ ] final spin of migrations
 - [ ] snapshot the database, just in case.
 - [ ] reboot all new hosts to use more RAM/cores and updated code
 - [ ] reboot all hubs to run new code
 - [ ] turn off all old compute machines
 - [ ] clean up google gce machines
...
# Cluster-wide change plan
 ...
 - [ ] vm fixes:
       - [ ] update code
       - [ ] ensure bup is newest, both in salvus account and system-wide.


# Fixing/polishing remaining issue with new system:

- [ ] test: when a `local_hub` socket dies, make sure that new connection is made automatically

- [ ] project close -- not tested since no UI for sending the mesg_project_close message.

- [ ] project move not tested since currently no UI for moving: need to write code for sending/rendering status of hosts.

- [ ] project should display a message suggesting user move project when restart fails
...
============================
 ...
 - [ ] deal with this:
        ...
        salvus@cloud3:/bup/bups/93d54f22-9351-4ffb-96c1-c5bdf1c4ba2d$ BUP_DIR=. bup ls master/latest/
        2013-11-13-002546.term     accessories.html           cell-phone-plans.html      index.html                 landing
        2013-11-13-005329.sagews   bring-your-own-phone.html  cell-phones                index.html.1               robots.txt
        accessories.cases.html     cell-phone-deals.html      content                    internet-devices           srvspub
        accessories.chargers.html  cell-phone-plans           etc                        internet-devices.html      www.t-mobile.com
...
- [x] new VM's -- at least for dc0 & dc1
      - [x] updated code
      - [x] delete storage user
      - [x] visudo with hashrings and bupstorage.py code.
      - [x] bup zpool configured as it should be for all vm's
      - [x] root ssh equivalence for rsync (for now -- maybe will use an rsync server at some point?).
      - [x] standard automatic zfs snapshotting system for the bup pool


- [x] bup storage: bup index doesn't work when something is fuse mounted

- [x] don't 'bup save' if no interesting files have changed.

- [x] modify database schema to use new system

        alter table projects add bup_location uuid;
        alter table projects add last_migrate_bup timestamp;
        alter table projects add last_migrate_bup_error varchar;
        alter table projects add abuser varchar;
...
- [ ] figure out server id's for all vm's, construct servers file, etc.
...
- [x] "project is opening" appears on all opening

- [x] idempotence if multiple clients request certain operations one after the other with same params -- combine them on the server side into one if in queue or current.  E.g., repeated "restarts".

- [x] save: make it so save ops are spaced out by a certain interval and automatically sync
- [x] decide what to do about these
      -->
        xx  replicate_projects_needing_replication:
        xx  replicate_all_with_errors:
        xx touch_project -- I deleted the code to automate making snapshots.  Should probably write code in storage_server.coffee to snapshot if...



- [x] update the 8 gce machines so I have my full cluster -- then start the final migration:

- [ ] loop for each project that has been touched since last time -- running on both cloud3 and cloud11 in parallel;
         do 0-8 on cloud11 and
         do 9-f on cloud3
   ...
   At the same time, finish stabilizing all the code on my laptop vm's.
    ...
    # on cloud11
    status=[];require('storage').migrate_bup_all(start:0, stop:25000, limit:50, status:status, cb:(e)->console.log("DONE!",e))

    # on cloud3
    status=[];require('storage').migrate_bup_all(retry_errors:true, start:25000, stop:45000, limit:50, status:status, cb:(e)->console.log("DONE!",e))


- [x] close_stale_projects
- [x] on project start, need to send quota information from hub, based on info in database: alter table projects add settings map<text,text>;
- [x] user quota info: use zfs part of status info to update this quota stuff for UI: 'Files and snapshots: 424MB (or 11%) this project's quota of 3.9GB (3.5GB are left)
- [x] client code: new way to get/view status of replicas and move -- disabled -- this is just too scary anyways.

----



- [x] write as fast-enough code that returns the status of the project on all replicas, where status is enough info to do the following intelligently

     - give me a hostname and port that works, doing a move if necessary; also will ensure that project is running on only one host.
     - restart the project server (to update code or reset everything); also ensure project is running on only one host.
     - safely move project from one host to another

In the database, we'll store (using quorum so slow but consistent):

 - Primary Location: serverid
 - Locations: {serverid:last_successful_save, ...}
 - Current state: stopped, starting, running, restarting, stopping, saving, moving-to-serverid
 - Time when entered current state
 - Last edit timestamp

Or in the DB we could do this:

      - last_save:   {serverid:timestamp, ...}
      - state:       {serverid:varchar, ...}



class GlobalProject:

   - start -- ensures started and returns host:port of localhub server, which definitely accepts connections.
   - move(target) -- attempts rsync of live files on current host to new host for up to n seconds, then starts on new host
   - state -- returns current state (and location)
   - save -- makes snapshot on current location, and sync it out

When doing save/sync, we'll provide the list of locations to sync to, based on db.  So adding new nodes doesn't *require* a rebalance... and we have the option to rebalance based on cpu + space usage, instead of random.  Consistent hashing will just be the default.

After a given action it always ensures that everything is stopped on all but the current location.

The client will be regularly informed of the above state



--

     - tell me the last time there was a successful snapshot

     - tell me quotao information
...
-------------
- [ ] in hub, this function "new_local_hub = (opts) ->    # cb" -- change it to "lock and queue"
- [ ] most of the LocalHub functions should be "lock and queue".
 ...
 cloud3
 ['c2446144-6a4f-4814-b18c-a17d256f7185','d63d3c2e-24a7-4ad2-8f25-3dd5db9c912f','f0560630-dfc9-40e0-b1bf-977445427db2']

 cloud11
 ['10039f83-9c56-4099-b34f-2dd901166d5d','8eeeb173-2ba4-445c-b100-b896c0521610' ]

 '16f04a6d-fb82-4bd5-a13b-90d8636f45a0' = bitcoin and big
