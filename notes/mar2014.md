...
# TOP PRIORITY
  ...
  - [ ] get needs to write to a temp file, then move it over -- destroy on fail.
  - [ ] storage_server: for next round of migrate_delete, need to delete storage/streams path first, due to the above...
        or at least change the sync code to check filesize.

  - [ ] coffee> debug: storage Client(undefined:44646).call({"mesg":{"event":"storage","project_id":"e53d0f98-f674-4569-aa8d-66fac50c61a5","action":"sync","id":"75d1
...
No row in table 'storage' matched condition '[object Object]' undefined
  ...
  - [ ] storage_server: make it so every database operation in gets retried until success

  - [ ] storage_server: make it so can be set so that all ZFS operations happen in order, instead of in parallel.
  ...
  - [ ] code to ensure storage server is started (as salvus!) when machine boots up (in `first_boot`*.py)
...
Backup thoughts:
   ...
   - [ ] implement a new offsite backup system, which could just involve --
   ...
   - [ ] try bup-based backup of everything:
