...
- NEXT goal: make it so I can 100% do cloud development in cloud, i.e., make it so my "cloud.sagemath" project
  is a complete running copy of cloud, using some port forward.  Why:
   ...
   Critical issues:
    ...
    - [ ] make it so projects can live in a subdirectory (*everything* is then in that subdirectory).
      ...
      -->    - [ ] There is a ton of code that hardcodes .sagemathcloud, so first fix that to all use an environment variable.
