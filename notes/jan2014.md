...
-->  - [ ] (0:30?) move MRC project back (in progress)

- [ ] lock so projects can't be moved automatically
-->      - [ ] (0:30?) make it so storage doesn't set location=undefined on close, instead only killing all procs; push out new code
      - [ ] (0:30?) change move UI to bring up a "temp. disabled" message, and email me if files vanished

 - [ ] (0:45?) add a `./build.py --build_zfs` option to make updating ZFS easier.

  - [ ] rebuild ZFS on base machines -- kernel upgraded!   careful of any reboot!



 - [ ] (1:00?) -- SUPER TOP PRIORITY -- ui for selecting from locations to move project; no automatic move (is that the case now? - maybe).
 ...
 - [ ] (1:00?) think about course homework workflow.
 - [ ] check -- is there something in help or faq about /scratch -- it is out of date.
