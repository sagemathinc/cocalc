...
- [ ]
y={};s.projects_needing_replication(cb:(e,n)->y.n=n)
x={};s.replication_errors(cb:(e,n)->x.n=n)

- [ ] latex editor should give up rendering errors after a certain number.  If there are thousands then browser hangs.

- [ ] sometimes hubs just go "crazy" trying to replicate, which makes the hub itself get very slow and not respond.  High cpu.  This has to be a bug in storage.
...
- [ ] setup and start running my encrypted in-office backup of all projects
     PLAN:
         - upgrade
         - build zfs
         - use image files: start with a single sparse 1TB file (?)
         - compress and dedup as much as possible... as usual
         - make it so my backup machine's key is trusted by all "storage" users (both on live machines and in base images)
         - setup database access
         - write function in storage that goes through and makes/updates backup of each project, etc.
...
- [ ] fix keyboard repeat on my laptop: defaults write -g ApplePressAndHoldEnabled -bool false
