...
- [ ] (0:45?) close_stale_projects has some problems:
     - it should space out each close by a few seconds, since doing all at once makes hub SLOW to respond to users.
     - it is deciding what is closed/open based on location, which doesn't get reset on close, so it does WAY too many closes!

--> - [ ] (0:45?) search hub logs for "Unhandled 'error' event" and fix things.
