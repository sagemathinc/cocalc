...
- [ ] worksheet printing -- minimal gui and release something:
    ...
    - [ ] (0:45?) provide a UI button/popup with title that prints
    - [ ] (0:15?) release and post message
...
- [ ] finish lxc.py script:
 ...
 - [ ] lxc: (??) optimization run .sagemathcloud/start_smc automatically on startup, when project is set.

 - [ ] lxc: install new ipython into sage
 - [ ] lxc: install_scripts in /usr/local/bin/
 - [ ] lxc: go through everything listed in build.py
...
- [ ] configure GCE machine
    - [ ] create debian 7 image
    - [ ] put it on the tinc vpn: 10.3.x.x for google hosts
           10.3.1.1
           10.3.2.1
...
- [ ] (1:00?) logging: make a new table that logs to the database each time a syncdoc (or other file) is opened by a client; this should be extremely useful for analytics, to see *what* people are using.
