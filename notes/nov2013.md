...
- [x] (1:00?) (0:35) start creation of a big unified bup backup of /mnt/home on each machine, in a central backup server.
          I tried backing up the 45GB on compute21a, and it compressed down to 4.5GB as a bup repo.
          Nice.  Making the index takes a *long* time, since there are nearly 4 million files.

- [x] (0:30?) (0:24) make html valid -- Harald Schilly + http://validator.w3.org/check?uri=https%3A%2F%2Fcloud.sagemath.com&charset=%28detect+automatically%29&doctype=Inline&group=0
...
- [x] (1:00?) (1:20) create a new backup vm on that 6TB backup disk, set to only copy snap files that are newer than "Nov 20" (say). Offsite for good the other 4TB disk on Monday.  The key idea was:
        mkdir -p ~/snap/%s/bup/; time rsync  -e "ssh -o StrictHostKeyChecking=no" -axvH --exclude *bupcache* --delete 10.1.%s.3:'$(find /mnt/snap/snap0/bup/* -type d -newermt "2013-11-29 1200" -prune)' ~/snap/%s/bup/"""%(i,i,i))

- [x] (0:30?) (0:37) make cloud.monitor.go() run on several different machines, spaced out by 5 minutes -- need some congruence condition input or something, to keep them out of sync.   OK, done, with a monitor on cloud3 (but only cloud3, due to firewall)
