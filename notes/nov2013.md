...
- [ ] (3:00?) rewrite hub to make direct connection instead of port forward.


- [ ] (1:30?) worksheets --> pdf (I promised something)
...
- [ ] qcow2 compression?  -- a one-time deal (see http://www.noah.org/wiki/KVM#Compress_a_QCOW2_image)
...
- [ ] spin up project container with given address, bind mount, etc.
      ...
      project_container.py start --project_id=project-id --ip_address=ip-address
      project_container.py stop --project_id=project-id

      the username will always be "smc".
