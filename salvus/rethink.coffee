...
class RethinkDB
    constructor : (opts={}) ->
        opts = defaults opts,
            ...
            password : undefined
            debug    : true
            driver   : 'dash'    # dash or native ; **Native is not done!**
            cb       : undefined
        dbg = @dbg('constructor')
        @_debug = opts.debug
            ...
            if not TermBase.run_native?  # only do this once!
                ...
                TermBase.run = (opts, cb) ->
                    if not cb?
                        cb = opts
                        opts = undefined
                    #winston.debug("run query")
                    f = (err, result) ->
                        if err
                            #winston.debug("got err=#{misc.to_json(err)}")
                            cb(err)
                        else
                            # TODO: what's needed here is actually *MUCH* more complicated than what is below; see the
                            # rethinkdbdash source code....
                            #winston.debug("query success")
                            if result?.toArray?
                                try
