...
DEFAULT_TOKEN_FILE  = 'data/syncstring.token'
DEFAULT_PORT        = 6000
...
# Network CLIENT
######################################################################
exports.syncstring_client = (opts) ->
    opts = defaults opts,
    ...
    constructor : (opts) ->
        ...
        async.series([
            ...
            if err
                ...
                @dbg("constructor", "connected SyncstringClient in #{misc.mswalltime(t)}ms")
                opts.cb(undefined, @)
        )
...
class SyncstringServer
    ...
    dbg: (f, m) =>
        winston.debug("SyncstringServer.#{f}: #{m}")
        ...
        server = net.createServer (socket) =>
            ...
            misc_node.unlock_socket socket, @secret_token, (err) =>
                if err
                    ...
                    handler = (type, mesg) =>
                        if type == "json"
                            @dbg("handler", "received mesg #{json(mesg)}")
