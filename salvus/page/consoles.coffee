...
class exports.Consoles
    constructor: (opts) ->
        opts = defaults opts,
            project_id       : required
            initial_consoles : undefined # if given, attempt to open these files on creation
        ...
        if opts.initial_consoles?
            for name in opts.initial_consoles
                @open(name)
    ...
    create_tab: (filename) =>
        ...
        link.find(".salvus-consoles-tab-filename").text(filename) #trunc(filename,15))
        ...
        @element.find(".salvus-consoles-content").append(editor.element.hide())
        @tabs[filename] = tab
        @update_counter()
        return tab
