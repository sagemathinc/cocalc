...
export class Actions extends BaseActions<LatexEditorState> {
  ...
  private _init_config_command(): void {
    // check fist 1000 lines if there is an engine configured
    // https://github.com/sagemathinc/cocalc/issues/2839
    if (this.engine_config !== undefined) return;

    const doc = this._get_doc();
    if (doc == null) {
      // console.log("_init_config_command doc undefined");
      return;
    }

    //console.log("_init_engine_config", doc);
    doc.eachLine(0, 1000, line => {
      if (this.engine_config !== undefined) return;
      if (line.text.startsWith("% !TeX program =")) {
    ...
    // console.log("this.engine_config = ", this.engine_config);
  }

  private _init_config(): void {
