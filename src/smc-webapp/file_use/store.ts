...
export interface FileUseState {
  errors: immutable.List<any>;
  file_use: immutable.Map<any, any>;
  notify_count: any;
}

export class FileUseStore extends Store<FileUseState> {
  private _account: any;
  private _account_id: any;
  private _cache: any;
  private _cache_init: any;
  ...
  get_file_info = (project_id: string, path: string) => {
    if (this._cache == null) {
      this._update_cache();
    }
    return this._cache != null
  ...
  get_project_info = (project_id: string) => {
    if (this._cache == null) {
      this._update_cache();
    }
    const v = {};
  ...
  _update_cache = () => {
    ...
    let v: any[] = [];
    const file_use_map = {};
    this.get("file_use").map((x, id) => {
      const y = x.toJS();
      y.search = this._search(y);
      this._process_users(y);
      v.push(y);
      return (file_use_map[id] = y);
    });
    const w0: any[] = [];
    const w1: any[] = [];
    const w2: any[] = [];
  ...
  get_all_unread = () => {
    return this.get_sorted_file_use_list().filter(x => x.is_unread);
  };

  get_all_unseen = () => {
    return this.get_sorted_file_use_list().filter(x => x.is_unseen);
  };
  ...
  get_active_users = (opts: ActiveUserOptions) => {
    ...
    const users = {};
    const now = webapp_client.server_time() - 0;
  ...
  get_video_chat_users(opts: VideoChatUsersOptions) {
    opts = misc.defaults(opts, {
      project_id: misc.required,
      path: misc.required,
      ttl: 120000
    }); // time in ms; if timestamp of video chat is older than this, ignore
    const users = {};
    const cutoff = webapp_client.server_time() - (opts.ttl as any);
    const fileusers = this.getIn([
      "file_use",
      sha1(opts.project_id, opts.path),
      "users"
    ]);
    if (fileusers != null) {
      fileusers.map((info, account_id) => {
        const timestamp = info.get("video");
        if (timestamp != null && timestamp - 0 >= cutoff) {
          return (users[account_id] = timestamp);
        }
      });
    }
    return users;
...
interface ActiveUserOptions {
  project_id?: string;
  path?: string;
  max_age_s?: number;
}

interface VideoChatUsersOptions {
  project_id: string;
  path: string;
  ttl?: number;
}
