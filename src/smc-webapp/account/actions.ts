...
import { GlobalInformationMessage } from "../global_info_message";
export { GlobalInformationMessage };
import {
  NAME_SYSTEM as SYSTEM_NOTIFICATIONS_NAME,
...
export class AccountActions extends Actions<AccountState> {
  ...
  constructor(name, redux) {
    ...
    misc.bind_methods(this, [
      "_init",
      "derive_system_notification_state",
      "set_user_type",
      "sign_in",
      "create_account",
      "delete_account",
  ...
  _init(store): void {
    store.on("change", this.derive_system_notification_state);
  }
  ...
  private derive_system_notification_state(store): void {
    ...
    const nh = store.getIn(["other_settings", "notification_high"]);
    const sna = this.redux.getActions<NotificationsState, NotificationsActions>(
      SYSTEM_NOTIFICATIONS_NAME
    );
    sna.update(nh, ni);
  }
