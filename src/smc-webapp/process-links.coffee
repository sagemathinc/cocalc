...
load_target = (target, switch_to, anchor) ->
    # get rid of "?something" in "path/file.ext?something"
    i = target.lastIndexOf('/')
    if i > 0 and '?' in target[i..]
        j = target[i..].indexOf('?')
        target = target[...(i + j)]
    projects_load_target(target, switch_to, false, true, anchor)
    ...
    if starts_with_cloud_url(href) and href.indexOf('/projects/') != -1
        ...
        y.click (e) ->
            ...
            if v.length > 1
                ...
                anchor = undefined
            load_target(decodeURI(target), not(e.which==2 or (e.ctrlKey or e.metaKey)), anchor)
            return false
        ...
        y.click (e) ->
            ...
            load_target(target, not(e.which==2 or (e.ctrlKey or e.metaKey)), anchor)
            return false
    else
