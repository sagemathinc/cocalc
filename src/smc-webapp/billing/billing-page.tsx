...
class BillingPage extends Component<Props> {
    ...
    return (
      <span>
        ...
        <b>
          ...
          considering purchasing a course subscription and need a short trial to
  ...
  private counts(): { cards: number; subs: number; invoices: number } {
    const cards =
      this.props.customer == null ? 0 : this.props.customer.sources.total_count;
    const subs =
      ...
      if (subs === 0) {
        ...
        return (
          <span>
            ...
            current subscriptions will run to completion, but will not renew. If
            you have any questions about subscriptions or billing (e.g., about
            purchase orders, using PayPal or wire transfers for non-recurring
            subscriptions above $50) please email <HelpEmailLink /> immediately.
          </span>
        );
      }
    } else if (subs === 0) {
...
const tmp = rclass(BillingPage);
export { tmp as BillingPage };
