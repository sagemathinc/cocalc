...
test("smoke test", () => {
  const rendered = shallow(<SkinnyError on_close={() => undefined} error_text={"Error testing text"}/>);
  expect(rendered).toMatchSnapshot();
});

test("test interaction", () => {
  const mock_on_close = jest.fn();
  const rendered = shallow(<SkinnyError on_close={mock_on_close} error_text={"Error testing text"}/>);
