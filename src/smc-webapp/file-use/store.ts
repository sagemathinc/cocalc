...
export class FileUseStore extends Store<FileUseState> {
  ...
  _course_discussion(y) {
    ...
    const path = y.path;
    const split_str = ".course-";
    const split_idx = path.indexOf(split_str);
    if (split_idx == -1) return;
    // two elements: the part before split_str, and the part afterwards to the end
    const course_discuss = [
      path.slice(0, split_idx),
      path.slice(split_idx + split_str.length)
    ];
    if (course_discuss.length != 2) return;
    const discuss = course_discuss[1];
    const account_id = discuss
      .split("-")
      .slice(-5)
      .join("-");
    if (!misc.is_valid_uuid_string(account_id)) return;
    const assignment = discuss.slice(0, -account_id.length - 1);
    y.orig_path = y.path;
