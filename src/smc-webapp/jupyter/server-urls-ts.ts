...
export function get_complete_url(
  project_id: string,
  path: any,
  code: any,
  cursor_pos: any
) {
  let s = `${get_server_url(
    project_id
  )}/kernels/complete?code=${encodeURIComponent(
    code
...
export function get_introspect_url(
  ...
  let s = `${get_server_url(
    project_id
  )}/kernels/introspect?code=${encodeURIComponent(
    code
  )}&path=${encodeURIComponent(path)}`;
  if (cursor_pos != null) {
    s += `&cursor_pos=${encodeURIComponent(cursor_pos)}`;
...
export function get_signal_url(project_id: string, path: any, signal: string) {
  return `${get_server_url(
    project_id
  )}/kernels/signal/${signal}?path=${encodeURIComponent(path)}`;
}

export function get_kernel_info_url(project_id: string, path: any) {
  return `${get_server_url(
    project_id
  )}/kernels/kernel_info?path=${encodeURIComponent(path)}`;
}
...
export function get_more_output_url(project_id: string, path: any, id: any) {
  return `${get_server_url(
    project_id
  )}/kernels/more_output?path=${encodeURIComponent(
    path
