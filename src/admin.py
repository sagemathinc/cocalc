...
if os.path.exists(AGENT):
    for X in open(AGENT).readlines():
        if 'SSH_AUTH_SOCK' in X:
            ...
            os.environ['SSH_AUTH_SOCK'] = X.split(';')[0][len(
                'SSH_AUTH_SOCK='):]
...
def email(msg='',
          subject='ADMIN -- %s' % DOMAINNAME,
          toaddrs='monitoring@sagemath.com',
          fromaddr='salvusmath@gmail.com'):
    log.info("sending email to %s", toaddrs)
    username = 'salvusmath'
    password = open(
        os.path.join(
            os.environ['HOME'],
            'smc/src/data/secrets/salvusmath_email_password')).read().strip()
    import smtplib
...
def run(args, maxtime=MAXTIME_S, verbose=True, stderr=True):
    ...
    if args and isinstance(args[0], list):
        return '\n'.join(
            [str(run(a, maxtime=maxtime, verbose=verbose)) for a in args])
    ...
    if maxtime:
        ...
        def timeout(*a):
            raise KeyboardInterrupt(
                "running '%s' took more than %s seconds, so killed" %
                (' '.join(args), maxtime))
...
class SH(object):
    ...
    def __getitem__(self, args):
        return run(
            [args] if isinstance(args, str) else list(args),
...
def process_status(pid, run):
    ...
    """
    fields = [
        '%cpu', '%mem', 'etime', 'pid', 'start', 'cputime', 'rss', 'vsize'
    ]
    v = run(
...
def init_data_directory():
    ...
    os.environ['PATH'] = os.path.join(DATA,
                                      'local/bin/') + ':' + os.environ['PATH']
...
class Component(object):
    ...
    def __repr__(self):
        return "Component %s with %s processes" % (self._id,
                                                   len(self._processes))
...
def parse_groupfile(filename):
    ...
    for r in open(filename).xreadlines():
        line = r.split('#')[
            0].strip()  # ignore comments and leading/trailing whitespace
...
def parse_hosts_file(filename):
    ...
    for r in open(filename).readlines():
        ...
        for h in hostnames:
            if len(h) < 1 or len(h) > 63 or not (h.replace('-', '').isalnum()):
                raise RuntimeError(
                    "invalid hostname: must be at most 63 characters from a-z, 0-9, or -"
                )
            if h in ip:
                ip[h].append(address)
            else:
    ...
    ip = dict([(host, list(sorted(set(addresses))))
               for host, addresses in ip.iteritems()])
    return ip, hn
...
class Hosts(object):
    ...
    def __init__(self, hosts_file, username=whoami, passwd=True,
        ...
        self._passwd = passwd
        self._ip_addresses, self._canonical_hostnames = parse_hosts_file(
            hosts_file)
    ...
    def __getitem__(self, hostname):
        ...
        if hostname in self._canonical_hostnames.keys(
        ):  # it is already a known ip address
            return [hostname]
    ...
    def password(self, retry=False):
        if not self._passwd:
            log.info(
                "Explicitly skipping asking for password, due to passwd=False option."
            )
            return self._password
        if self._password is None or retry:
            import getpass
            self._password = getpass.getpass(
                "%s's password: " % self._username)
        return self._password
    ...
    def map(self, callable, hostname, parallel=True, **kwds):
        ...
        def f(address, **kwds):
            return ((address, self.hostname(address)),
                    self._do_map(callable, address, **kwds))

        if parallel:
            return misc.thread_map(
                f, [((address, ), kwds) for address in self[hostname]])
        else:
    ...
    def ping(self, hostname='all', timeout=3, count=3, parallel=True):
        ...
        """
        v = self.map(
            ping, hostname, timeout=timeout, count=count, parallel=parallel)
        return [x for x in v
                if x[1] is not False], [x for x in v if x[1] is False]
    ...
    def exec_command(self,
        ...
        def f(hostname):
            try:
                ...
                return {
                    'stdout': '',
                    'stderr': 'Error connecting -- %s: %s' % (hostname, msg)
                }
    ...
    def _exec_command(self,
        ...
        try:
            ...
            chan = self.ssh(
                hostname, username=username, timeout=timeout,
                use_cache=False).get_transport().open_session()
        stdin = chan.makefile('wb')
        stdout = chan.makefile('rb')
        stderr = chan.makefile_stderr('rb')
        cmd = ('sudo -S bash -c "%s"' % command.replace(
        ...
        if not wait:
            return {
                ...
                'stderr': None,
                'exit_status': None,
                'note': "wait=False: '%s'" % cmd
            }
        while not stdout.channel.closed:
            time.sleep(0.05)
            if time.time() - start >= timeout:
                raise RuntimeError("on %s@%s command '%s' timed out" %
                                   (self._username, hostname, command))
        return {
            'stdout': stdout.read(),
            'stderr': stderr.read(),
    ...
    def public_ssh_keys(self, hostname, timeout=5):
        return '\n'.join([
            x['stdout'] for x in self.exec_command(
                hostname, 'cat .ssh/id_rsa.pub', timeout=timeout).values()
        ])

    def git_pull(self, hostname, repo=GIT_REPO, timeout=60):
        return self(
            hostname, 'cd salvus && git pull %s' % repo, timeout=timeout)

    def build(self, hostname, pkg_name, timeout=250):
        return self(
            hostname,
            'cd $HOME/smc/src && source ./smc-env && ./build.py --build_%s' %
            pkg_name,
            timeout=timeout)
    ...
    def apt_upgrade(self, hostname):
        ...
        try:
            return self(
                hostname,
                'ufw --force disable && apt-get update && apt-get -y upgrade',
                sudo=True,
                timeout=120)
    ...
    def apt_install(self, hostname, pkg):
        ...
        try:
            return self(
                hostname,
                'ufw --force disable && apt-get -y --force-yes install %s' %
                pkg,
                sudo=True,
                timeout=120)
        finally:
    ...
    def ufw(self, hostname, commands):
        if self[hostname] == ['127.0.0.1']:
            ...
            return
        cmd = ' && '.join(
            ['/home/salvus/smc/src/scripts/ufw_clear'] + ['ufw disable'] +
            ['ufw default allow incoming'] + ['ufw default allow outgoing'] +
            ['ufw --force reset'] + ['ufw ' + c for c in commands] +
    ...
    def put(self, hostname, local_filename, remote_filename=None, timeout=5):
        ...
        for hostname in self[hostname]:
            ...
            log.info('put: %s --> %s:%s', local_filename, hostname,
                     remote_filename)
    ...
    def putdir(self,
        ...
        for hostname in self[hostname]:
            ...
            for dirpath, dirnames, filenames in os.walk(local_path):
                ...
                self._mkdir(sftp, os.path.join(remote_containing_path,
                                               dirpath))
                for name in filenames:
                    local = os.path.join(dirpath, name)
                    remote = os.path.join(remote_containing_path, dirpath,
                                          name)
    ...
    def _mkdir(self, sftp, path, mode=0o40700):
        try:
            ...
            if not S_ISDIR(sftp.stat(path).st_mode):
                raise IOError("remote '%s' (on %s) exists and is not a path" %
                              (path, hostname))

    def mkdir(
            self, hostname, path, timeout=10, mode=0o40700
    ):  # default mode is restrictive=user only, on general principle.
        for hostname in self[hostname]:
            ssh = self.ssh(hostname, timeout=timeout)
            sftp = ssh.open_sftp()
...
class Monitor(object):
    ...
    def compute(self):
        ...
        for k, v in self._hosts(
                'compute', c, wait=True, parallel=True,
                timeout=120).iteritems():
    ...
    def hub(self):
        ...
        for k, v in self._hosts(
                'hub', cmd, wait=True, parallel=True, timeout=60).iteritems():
            ...
            try:
                d['block'] = int(
                    v['stdout'].splitlines()[3].split()[-1].rstrip('ms'))
                if d['block'] > 15000:
                    d['status'] = 'down'  # demands attention!
            except:
                pass
        ...
        def f(x, y):
            ...
            if 'loadavg' in x and 'loadavg' in y:
                return -cmp(
                    float(x['loadavg'].split()[0]),
                    float(y['loadavg'].split()[0]))
            return -1
    ...
    def load(self):
        ...
        ans = []
        for k, v in self._hosts(
                'all', 'nproc && uptime', parallel=True, wait=True,
                timeout=80).iteritems():
            d = {'host': k[0]}
    ...
    def disk_usage(self, hosts='all', disk_threshold=DISK_THRESHOLD):
        ...
        ans = []
        for k, v in self._hosts(
                hosts, cmd, parallel=True, wait=True, timeout=30).iteritems():
            d = {'host': k[0], 'service': 'disk_usage'}
            percent = int(
                (v.get('stdout', '100') + ' 0').split()[0].strip().strip('%'))
            d['percent'] = percent
    ...
    def dns(self, hosts='all', rounds=1):
        ...
        """
        cmd = '&&'.join(
            ["host -v google.com > /dev/null"] * rounds) + "; echo $?"
        ans = []
        exclude = set(
            [])  # set(self._hosts['cellserver'])  # + self._hosts['webdev'])
        h = ' '.join(
            [host for host in self._hosts[hosts] if host not in exclude])
        if not h:
            return []
        for k, v in self._hosts(
                h, cmd, parallel=True, wait=True, timeout=30).iteritems():
    ...
    def stats(self, timeout=90):
        ...
        )  # see http://stackoverflow.com/questions/19268548/python-ignore-certicate-validation-urllib2
    ...
    def print_status(self, all=None, n=9):
        ...
        print "%s projects running" % (sum(
            [x.get('nprojects', 0) for x in vcompute]))
...
class Services(object):
    def __init__(self,
        ...
        self._username = username
        self._hosts = Hosts(
            os.path.join(path, 'hosts'),
            username=username,
            passwd=passwd,
            password=password)

        self._services, self._ordered_service_names = parse_groupfile(
            os.path.join(path, 'services'))
        del self._services[None]

        self.monitor = Monitor(
            Hosts(
                os.path.join(path, 'hosts'), username=username, passwd=False),
            services=self)
        ...
        def hostopts(service, query='all', copy=True):
            ...
            return sum([[(h, dict(opts) if copy else opts)
                         for h in self._hosts[query] if h in restrict]
                        for query, opts in self._services[service]], [])

        self._options = dict([(service, hostopts(service))
                              for service in self._ordered_service_names])

    def _all(self, callable, reverse=False):
        names = self._ordered_service_names
        return dict([(s, callable(s))
                     for s in (reversed(names) if reverse else names)])

    def start(self, service, host='all', wait=True, parallel=False, **opts):
        if service == 'all':
            return self._all(
                lambda x: self.start(x, host=host, wait=wait, **opts),
                reverse=False)
        return self._action(
            service, 'start', host, opts, wait=wait, parallel=parallel)

    def stop(self, service, host='all', wait=True, parallel=False, **opts):
        if service == 'all':
            return self._all(
                lambda x: self.stop(x, host=host, wait=wait, **opts),
                reverse=True)
        return self._action(
            service, 'stop', host, opts, wait, parallel=parallel)

    def status(self, service, host='all', wait=True, parallel=False, **opts):
        if service == 'all':
            return self._all(
                lambda x: self.status(x, host=host, wait=True, **opts),
                reverse=False)
        return self._action(
            service, 'status', host, opts, wait=True, parallel=parallel)
    ...
    def restart(self,
        ...
        return self._action(
            service, 'restart', host, opts, wait, parallel=parallel)
    ...
    def _action(self, service, action, host, opts, wait, parallel):
        ...
        w = [((name, action, address, options, db_string(address), wait), {})
    ...
    def _hostopts(self, service, hostname, opts):
        ...
        return [(h, dict(o)) for h, o in self._options[service]
                if h in hosts and opts1.issubset(
                    set([(x, y) for x, y in o.iteritems() if x in opts]))]
    ...
    def _do_action(self, name, action, address, options, db_string, wait):
        ...
        for t in ['hub', 'nginx', 'proxy']:
            ...
            if s in options:
                ...
                options[s] = [
                    dict(x) for x in options[s]
                    if self.ip_address_to_dc(x['ip']) == dc
                ]
        ...
        cmd = "import admin; print admin.%s(%s**%r).%s()" % (name, db_string,
                                                             options, action)

        ret = self._hosts.python_c(
            address, cmd, sudo=sudo, timeout=timeout, wait=wait)
