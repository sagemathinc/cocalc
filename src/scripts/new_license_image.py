...
if prev == "":
    ...
    prev = os.popen(
        "ls -1t %s/*.qcow2|head -1" % VM_PATH).read().strip().rstrip('.qcow2')
...
cmd("virt-install --connect qemu:///system --cpu host --network network:default,model=virtio --name %s --vcpus=4 --ram 4000 --import --disk %s,device=disk,bus=virtio,format=qcow2,cache=writeback --noautoconsole --graphics vnc,port=12505"
    % (next, next_path))
