...
) ON CONFLICT(id) DO UPDATE SET blob=EXCLUDED.blob, expire=EXCLUDED.expire, created=EXCLUDED.created, project_id=EXCLUDED.project_id, last_active=EXCLUDED.last_active, count=EXCLUDED.count, size=EXCLUDED.size, gcloud=EXCLUDED.gcloud, backup=EXCLUDED.backup;
