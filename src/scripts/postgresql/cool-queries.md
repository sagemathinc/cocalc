...
Problems people are having right now:
    ...
    select NOW() - time as timeago, left(account_id::VARCHAR,6), left(error,120) as error from client_error_log where error like 'Error saving file%' order by time desc limit 50;
