...
def chown_all(project_id):
    ...
    cmd("zfs set snapdir=hidden %s" %
        home(project_id).lstrip('/'))  # needed for historical reasons
    id = uid(project_id)
...
def write_info_json(project_id, host='', base_url=''):
    ...
    path = os.path.join(
        home(project_id), '.sagemathcloud' + ('-local' if base_url else ''))
...
def killall_user(project_id):
    u = uid(project_id)
    os.system("pkill -u %s; sleep 1; pkill -9 -u %s; killall -u %s" %
              (u, u, username(project_id)))
...
def copy_skeleton(project_id):
    ...
    os.system(
        "rsync -axH --update /home/salvus/salvus/salvus/scripts/skel/ %s/" %
        h)  # update so we don't overwrite newer versions
...
def cgroup(project_id, cpu=1024, memory='8G'):
    ...
    cmd('echo "%s" > /sys/fs/cgroup/memory/%s/memory.limit_in_bytes' % (memory,
                                                                        uname))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--kill",
        help="kill all processes owned by the user",
        default=False,
        action="store_const",
        const=True)
    parser.add_argument(
        "--umount",
        help="run umount on the project's home as root",
        default=False,
        action="store_const",
        const=True)
    parser.add_argument(
        "--skel",
        help=
        "rsync /home/salvus/salvus/salvus/scripts/skel/ to the home directory of the project",
        default=False,
        action="store_const",
        const=True)
    parser.add_argument(
        "--create",
        help="create the project user",
        default=False,
        action="store_const",
        const=True)
    parser.add_argument(
        "--base_url", help="the base url (default:'')", default="", type=str)
    parser.add_argument(
        "--host",
        help="the host ip address on the tinc vpn (default: auto-detect)",
        default="",
        type=str)
    parser.add_argument(
        "--chown",
        help="chown all the files in /projects/projectid",
        default=False,
        action="store_const",
        const=True)
    parser.add_argument(
        "--cgroup",
        help=
        "put project in given control group (format: --cgroup=cpu:1024,memory:10G)",
        default="",
        type=str)
    ...
    if args.create:
        ...
        write_info_json(
            project_id=args.project_id, host=args.host, base_url=args.base_url)
