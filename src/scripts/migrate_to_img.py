...
def migrate_to_img(project_id):
    ...
    cmd("zfs send -Dv %s@%s | lz4c - > project-%s-%s-to-%s.lz4" %
        (filesystem2, now, project_id, now, now))


if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Create sparse image zfs format of project, with fixed uid"
    )

    parser.add_argument(
        "project_id",
        help="UUID of project stored locally on this computer",
        type=str,
        nargs="+")
