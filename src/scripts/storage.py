...
class MultiHashRing(object):
    ...
    def _load(self):
        v = [
            x for x in open(self._topology_file).readlines()
...
def migrate_project_to_storage(src, new_only):
    ...
    if new_only and not is_new:
        log.info("skipping %s (%s) since it already exists (and new_only=True)"
                 % (src, project_id))
        return
    ...
    # rsync data over
    cmd("rsync -Hax --delete --exclude .forever --exclude .bup --exclude .zfs %s/ %s/"
        % (src, home))
...
def mount(project_id):
    cmd('sudo zfs set mountpoint=%s %s' % (path_to_project(project_id),
...
def quota(project_id, new_quota=''):
    ...
    dataset = dataset_name(project_id)
    if new_quota:
        Pool(processes=len(locs)).map(cmd, [
            "ssh %s 'sudo zfs set quota=%s %s'" % (host, new_quota, dataset)
            for host in locs
        ])
        return new_quota
    else:
        try:
            ...
            for host in locs:
                try:
                    out = cmd(
                        "ssh %s 'sudo zfs get quota %s'" % (host, dataset))
                    break
                except:
                    pass
            if out is None:
                raise RuntimeError(
                    "unable to determine quota of %s" % project_id)
...
def create_user(project_id):
    ...
    cmd("useradd -u %s -g %s -o -d %s %s" %
        (id, id, path_to_project(project_id), name))
...
def list_snapshots(project_id, host=''):
    ...
    c = "sudo zfs list -r -t snapshot -o name -s creation %s" % dataset_name(
        project_id)
    try:
        if not host:
            v = cmd(c)
    ...
    v = [
        x[n + 1:] for x in v
        if "Warning" not in x and x.startswith('projects/')
    ]  # eliminate any ssh key warnings.
    v.sort()
    return v
...
def status(project_id, long_format=False, exclude_system=True):
    ...
    home = path_to_project(project_id)
    n = len(home) + 1
    c = "sudo zfs diff -H%s `sudo zfs list -r -t snapshot -o name -s creation %s|tail -1` %s" % (
        'Ft' if long_format else '', d, d)
    if exclude_system:

        def exclude(x):
            return x == '' or x.startswith('.forever') or x.startswith(
                '.sagemathcloud')
    else:
    ...
    if long_format:
        ...
        for x in cmd(c).splitlines():
            ...
            v[filename] = {
                'time': t,
                'type': ZFS_FILE_TYPES[typ],
                'action': ZFS_CHANGES[action]
            }
    else:
...
def snapshot(project_id, name='', force=False):
    ...
    if not force:
        if len(status(project_id, long_format=False)) == 0:
            ...
            return ''
    name = "%s@%s%s" % (dataset_name(project_id),
...
def snapshot_cache_file(host=None):
    ...
    return os.path.abspath(
        os.path.join(CACHE_PATH, 'snapshots-%s.json' % host))
...
def update_snapshot_cache(host=None):
    ...
    log.info("Got list from ZFS (%s seconds); now parsing and saving...",
             time.time() - t)
    t = time.time()
    ...
    log.info("Finished parsing and saving snapshot cache (%s seconds)",
             time.time() - t)
...
def update_all_snapshot_caches():
    ...
    pool = Pool(processes=len(hosts))
    log.info(
        "Updating all snapshot caches on all nodes.  This could take up to about 5-10 minutes."
    )
...
def work_to_sync(send=True, destroy=False):
    ...
    for project_id, newest in cache.iteritems():
        ...
        if send:
            ...
            available_dcs = set(hashring.topology.keys(
            ))  # we are still allowed to take a host from these data centers
            for dest, snap in stale_hosts:
                ...
                if dc == datacenter or dc in available_dcs:
                    work.append({
                        ...
                        'src': src,
                        'dest': dest,
                        'snap_src': newest_snap,
                        'snap_dest': snap
                    })
                    if dc != datacenter:
                        available_dcs.remove(
                            dc
                        )  # remember not to take any more hosts from this datacenter
...
def dump_to_database(outfile):
    ...
    for host in all_hosts():
        for project_id, snapshots in snapshot_cache(host).iteritems():
            if project_id not in c:
                c[project_id] = {}
            c[project_id][host] = list(
                reversed(sorted([str(x) for x in snapshots])))
...
def send_multi(project_id, destinations, force=True, snap_src=None,
    ...
    snap_destinations = newest_snapshot(
        project_id, hosts=destinations, timeout=timeout)

    log.debug("src: %s, dest: %s", snap_src, snap_destinations)

    for s in set(snap_destinations.itervalues()):
        snap_dest = dataset + '@' + s if s else ''
        _send_multi(project_id, [
            dest for dest in snap_destinations if snap_destinations[dest] == s
...
def all_local_project_ids():
    ...
    """
    v = []
    for x in os.popen("sudo zfs list").readlines(
    ):  # cmd takes too long for some reason given huge output
        w = x.split()
            ...
            while len(pids) < pool_size and len(project_ids) > 0:
                ...
                try:
                    ...
                    if pool_size > 1:
                        ...
                        if pid:
                            log.debug(
                                "FORKED %s to handle %s" % (pid, project_id))
                            pids[pid] = project_id
                        else:
                            # child
                            try:
...
def activity_watcher(active_path='/home/storage/active', ignore_dot=True):
    ...
    def add(pathname):
        ...
        cmd("mkdir -p '%s'; touch '%s'; chown -R storage. '%s'" %
...
def replicate_active_watcher(min_time_s=60,
                             active_path='/home/storage/active'):
    """
    ...
    while True:
        if os.path.exists(active_path):
            for project_id in os.listdir(active_path):
                now = time.time()
                if pids.get(project_id, False) or last_attempt.get(
                        project_id, 0) + min_time_s >= now:
                    continue
...
if __name__ == "__main__":
    ...
    parser_migrate = subparsers.add_parser(
        'migrate',
        help=
        'create or update dataset (and user) corresponding to the given home directories'
    )
    parser_migrate.add_argument(
        "--new_only",
        help="if dataset already created, do nothing (default: False)",
        default=False,
        action="store_const",
        const=True)
    parser_migrate.add_argument(
    ...
    parser_snapshot = subparsers.add_parser(
        'snapshot',
        help=
        'make a new snapshot of the given project; if only files in ~/.forever and ~/.sagemathcloud change, no snapshot is created'
    )
    parser_snapshot.add_argument("project_id", help="project id", type=str)
    parser_snapshot.add_argument(
        "name",
        help="appended to end of snapshot timestamp",
        type=str,
        default='',
        nargs='?')
    ...
    parser_status = subparsers.add_parser(
        'status',
        help=
        'the files that have been modified/changed/created since the last snapshot (JSON format)'
    )
    parser_status.add_argument(
        "--long",
        help=
        "instead output dictionary {filename:{time:?,type:?,action:?},....}",
        default=False,
        action="store_const",
        const=True)
    ...
    def _snapshots(args):
        print_json({
            ...
            args.project_id,
            'snapshots':
            list_snapshots(project_id=args.project_id, host=args.host)
        })

    parser_snapshots = subparsers.add_parser(
        'snapshots', help='list in order the snapshots of the given project')
    ...
    parser_snapshots_cache = subparsers.add_parser(
        'update_snapshot_cache',
        help=
        'regenerate the file ~/cache/snapshots-ip_address.json (on unloaded system, takes about 5 seconds per 1000 projects)'
    )
    parser_snapshots_cache.add_argument(
        "--all",
        help=
        "update snapshot caches on every node in the cluster and copy the caches back to this host (doesn't copy them to every host)",
        default=False,
        action="store_const",
        const=True)
    ...
    parser_repair = subparsers.add_parser(
        'repair',
        help=
        'Push out any projects that needs to be pushed out (from here, to at least one node in each data center), and if safe, also deletes projects that no longer need to be here.  Run this one at a time on each machine. '
    )
    parser_repair.add_argument(
        ...
        default=6,
        help=
        "number of projects to simultaneously replicate; this has a major impact on the load repair puts on a machine; (default: 6)"
    )
    parser_repair.add_argument(
        "--no_cache_update",
    ...
    def _newest_snapshot0(args):
        v = newest_snapshot(
            ...
            hosts=locations(args.project_id),
            timeout=args.timeout)
        print_json({'project_id': args.project_id, 'newest_snapshots': v})

    parser_newest_snapshot = subparsers.add_parser(
        'newest_snapshot',
        help='output json object giving newest snapshot on each available host'
    )
    parser_newest_snapshot.add_argument(
        "project_id", help="project id", type=str)
    parser_newest_snapshot.add_argument(
        "--timeout",
        dest='timeout',
        type=int,
        default=60,
        help="timeout to declare host not available")
    parser_newest_snapshot.set_defaults(func=_newest_snapshot0)

    def _usage(args):
        print_json({
            'project_id': args.project_id,
            'usage': usage(args.project_id)
        })

    parser_usage = subparsers.add_parser(
        'usage', help='disk usage information about the project')
    ...
    parser_umount = subparsers.add_parser(
        'umount', help='unmount the filesystem')
    ...
    parser_quota = subparsers.add_parser(
        'quota', help='get or set the quota for a project')
    ...
    parser_quota.add_argument(
        ...
        type=str,
        default='',
        help="if given set the quota to this value (e.g., 10G)")
    ...
    parser_send = subparsers.add_parser(
        'send',
        help=
        'send latest UTC iso date formated snapshot of project to remote storage servers (this overwrites any changes to the targets)'
    )
    parser_send.add_argument("project_id", help="project id", type=str)
    parser_send.add_argument(
        "dest", help="ip address of destination", type=str, nargs="+")
    ...
    parser_replicate = subparsers.add_parser(
        'replicate',
        help=
        'replicate project out from here to all its replicas, as defined using consistent hashing'
    )
    parser_replicate.add_argument(
        "project_id", help="project id", type=str, nargs="*")
    parser_replicate.add_argument(
        "--pool_size",
        dest='pool_size',
        type=int,
        default=1,
        help=
        "number of projects to replicate out at once (this doesn't work very well due to multiprocessing misery!!)"
    )
    parser_replicate.add_argument(
        "--all",
        help="replicate all locally stored projects",
        default=False,
        action="store_const",
        const=True)
    ...
    parser_activity = subparsers.add_parser(
        'activity',
        help=
        'watch the /projects directory, and when any file or path changes, touch the file active_path/project_id.'
    )
