...
def newest_snapshot(fs):
    out = cmd(
        "sudo /sbin/zfs list -r -t snapshot -o name -s creation %s |tail -1" %
        fs)
...
def mount(mountpoint, fs):
    if cmd("sudo /sbin/zfs get -H mounted %s" %
           fs).split()[2] == 'yes' and cmd(
               "sudo /sbin/zfs get -H mountpoint %s" %
               fs).split()[2] == mountpoint:
        # already done.
        return
...
class Stream(object):
    def __init__(self, project, path):
        ...
        self.start, self.end = self.filename.split('.')[0].split(
            '--')  # [date]--[date].zvol.lz4
    ...
    def apply(self):
        ...
        if self.project.project_pool_is_imported():
            raise RuntimeError(
                "cannot receive stream while pool already imported")
        cmd("cat '%s' | lz4c -d - | sudo /sbin/zfs recv -F %s" %
            (self.path, self.project.zvol_fs))
...
def optimal_stream_sequence(v):
    ...
    while True:
        ...
        if len(v) > 0:
            del v[
                0]  # delete first element -- it's not the end of a valid sequence.
...
class Project(object):
    def __init__(self,
        ...
        self.sagemathcloud_template_fs = os.path.join(
            self.sagemathcloud_base_fs, 'template')
        self.sagemathcloud_fs = os.path.join(self.sagemathcloud_base_fs,
                                             project_id)
    ...
    def create_user(self):
        u = self.uid
        cmd('sudo /usr/sbin/groupadd -g %s -o %s' % (u, self.username),
            ignore_errors=True)
        cmd('sudo /usr/sbin/useradd -u %s -g %s -o %s -d %s -s %s' %
            (u, u, self.username, self.project_mnt, self.login_shell),
            ignore_errors=True)
    ...
    def create(self, quota=DEFAULT_QUOTA):
        ...
        cmd(
            "sudo /sbin/zfs set sync=disabled %s" % self.zvol_fs
        )  # CRITICAL to set sync=disabled -- or it is a million times slower
        log("create a pool on the zvol")
        cmd("sudo /sbin/zpool create %s -m '%s' %s" %
            (self.project_pool, self.project_mnt, self.zvol_dev))
    ...
    def import_pool(self):
        ...
        if len(optimal_stream_sequence(
                self.streams())) == 0 and not filesystem_exists(self.zvol_fs):
    ...
    def kill(self, grace_s=0.25):
        ...
        for i in range(MAX_TRIES):
            cmd("sudo /usr/bin/pkill -u %s; sleep %s; sudo /usr/bin/pkill -9 -u %s"
                % (self.uid, grace_s, self.uid),
                ignore_errors=True)
            n = self.num_procs()
    ...
    def pids(self):
        return [
            int(x)
            for x in cmd("pgrep -u %s" % self.uid, ignore_errors=True).replace(
                'ERROR', '').split()
        ]
    ...
    def recv_streams(self):
        ...
        if self.project_pool_is_imported():
            raise RuntimeError(
                'cannot recv streams since project pool is already imported')
    ...
    def send_streams(self):
        ...
        snaps = snapshots(self.zvol_fs)
        big_stream_ends = set(
            [x.end for x in v if x.size_mb() >= self.stream_thresh_mb])
        start = end
        ...
        target = os.path.join(self.stream_path,
                              "%s--%s%s" % (start, end, STREAM_EXTENSION))
        try:
            log("sending new stream: %s" % target)
            try:
                out = cmd(
                    "sudo /sbin/zfs send -v %s | lz4c - > %s.partial && mv %s.partial %s"
    ...
    def compact_zvol(self):
        ...
        try:
            ...
            cmd("sudo /usr/local/bin/compact_zvol %s" % tmp,
                ignore_errors=True)
        finally:
            cmd("sudo /sbin/zfs set compression=lz4 %s" % self.project_pool)
    ...
    def update_sagemathcloud_template(self):
        ...
        cmd("sudo /sbin/zfs set mountpoint=/%s %s; sudo /sbin/zfs mount %s" %
            (self.sagemathcloud_template_fs, self.sagemathcloud_template_fs,
    ...
    def newest_sagemathcloud_template_snapshot(self):
        ...
        v = os.listdir(
            os.path.join('/' + self.sagemathcloud_template_fs, '.zfs',
                         'snapshot'))
        v.sort()
        return v[-1]
    ...
    def update_sagemathcloud_fs(self):
        ...
        if not filesystem_exists(self.sagemathcloud_fs):
            ...
            cmd("rsync -axH  /%s/.zfs/snapshot/%s/ %s@localhost:/%s/" %
                (self.sagemathcloud_template_fs, snap, self.username,
    ...
    def destroy_sagemathcloud_fs(self):
        ...
        cmd("sudo /sbin/zfs destroy -r %s" % self.sagemathcloud_fs,
            ignore_errors=True)  # error if doesn't exist or user on it.
    ...
    def ensure_ssh_access(self):
        ...
        cmd("sudo /usr/local/bin/ensure_ssh_access.py %s %s" %
            (self.project_mnt, SSH_ACCESS_PUBLIC_KEY))
        cmd("sudo /usr/local/bin/ensure_file_exists.py %s %s/.bashrc" %
            (BASHRC_TEMPLATE, self.project_mnt))
        cmd("sudo /usr/local/bin/ensure_file_exists.py %s %s/.bash_profile" %
            (BASH_PROFILE_TEMPLATE, self.project_mnt))
    ...
    def cgroup(self, memory_G, cpu_shares, core_quota):
        ...
        cmd("sudo /usr/local/bin/cgroup.py %s %s %s %s" %
            (self.username, memory_G, cpu_shares, cfs_quota))
    ...
    def migrate_from(self, host):
        ...
        try:
            ...
            if q == 0:
                ...
                if q == 0:
                    raise RuntimeError(
                        "unable to mount remote filesystem and get quota")
                else:
                    we_mounted_it = True
            ...
            log("doing rsync")
            rsync = 'rsync -axH --exclude .zfs --exclude .npm --exclude .sagemathcloud --exclude .node-gyp --exclude .cache --exclude .forever --exclude .ssh root@%s:/projects/%s/ /%s/' % (
                host, self.project_id, self.project_mnt)

            cmd("sudo /bin/su - %s -c '%s'" % (self.username, rsync),
                timeout=60 * 60)  # can't take more than an hour

            log("umounting")
            if we_mounted_it:
                umnt = 'ssh %s "sudo zfs umount projects/%s"' % (
                    host, self.project_id)
                cmd(umnt, ignore_errors=True,
                    timeout=30)  # not a big deal if unmount isn't guaranteed
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--login_shell",
        help="the login shell used when creating user (default:'/bin/bash')",
        default="/bin/bash",
        type=str)
    parser.add_argument(
        "--stream_path",
        help=
        "directory where streams are stored for this project(default: '/[pool]/streams/[project_id]')",
        default="",
        type=str)
    ...
    parser_import_pool = subparsers.add_parser(
        'import_pool',
        help=
    ...
    parser_recv_streams = subparsers.add_parser(
        'recv_streams',
        help=
        'receive any streams that have not yet been applied to the image filesystem; error if zpool is mounted'
    )
    parser_recv_streams.set_defaults(func=lambda args: project.recv_streams())

    parser_send_streams = subparsers.add_parser(
        'send_streams',
        help='updates streams to reflect state of image filesystem')
    ...
    parser_sagemathcloud.add_argument(
        ...
        help="if given, update the template itself",
        default=False,
        action="store_const",
        const=True)
    ...
    parser_ensure_ssh_access = subparsers.add_parser(
        'ensure_ssh_access',
        help='add public key so user can ssh into the project')
    parser_ensure_ssh_access.set_defaults(
        func=lambda args: project.ensure_ssh_access())

    parser_cgroup = subparsers.add_parser(
        'cgroup', help='configure cgroup for this user')
    ...
    parser_cgroup.add_argument(
        "--core_quota",
        dest="core_quota",
        help="max number of cores -- can be float (default: '%s')" %
        DEFAULT_CORE_QUOTA,
        type=float,
    ...
    parser_replicate = subparsers.add_parser(
        'replicate',
        help=
        'directly send streams to another host via rsync (instead of database)'
    )
    parser_replicate.add_argument(
        "--delete",
        help="deletes any files on target not here (DANGEROUS); off by default",
        dest="delete",
        default=False,
        action="store_const",
        const=True)
    parser_replicate.add_argument("target", help="target hostname", type=str)
    parser_replicate.set_defaults(
        func=lambda args: project.replicate(args.target, delete=args.delete))

    parser_close = subparsers.add_parser(
        'close',
        help='send_streams, unmount, destroy images, etc., leaving only streams'
    )
    parser_close.add_argument(
        "--nosend_streams",
        help="if given, don't send_streams first: DANGEROUS",
        default=False,
        action="store_const",
        const=True)
    parser_close.set_defaults(
        func=lambda args: project.close(send_streams=not args.nosend_streams))

    parser_kill = subparsers.add_parser(
        'kill', help='Kill all processes running as this user.')
    ...
    parser_destroy_zvol_fs = subparsers.add_parser(
        'destroy_zvol_fs',
        help=
        'export project pool and destroy the image filesystem, leaving only streams'
    )
    parser_destroy_zvol_fs.set_defaults(
        func=lambda args: project.destroy_zvol_fs())

    parser_destroy_streams = subparsers.add_parser(
        'destroy_streams', help='destroy all streams stored locally')
    parser_destroy_streams.set_defaults(
        func=lambda args: project.destroy_streams())

    parser_compact_zvol = subparsers.add_parser(
        'compact_zvol',
        help=
        'compact the zvol, so exporting it will take way less space in case it has blown up and shrunk -- would only make sense after deleting snapshots'
    )
    parser_compact_zvol.set_defaults(func=lambda args: project.compact_zvol())

    parser_snapshot_pool = subparsers.add_parser(
        'snapshot_pool', help='snapshot the project zpool')
    parser_snapshot_pool.add_argument(
        "--name",
        dest="name",
        help="name of snapshot (default: ISO date)",
        type=str,
        default='')
    parser_snapshot_pool.set_defaults(
        func=lambda args: project.snapshot_pool(args.name))
    ...
    parser_snapshots = subparsers.add_parser(
        'snapshots',
        help='show list of snapshots of the given project pool (JSON)')
    ...
    if not args.stream_path:
        args.stream_path = '/' + os.path.join(args.pool, 'streams',
                                              args.project_id)
