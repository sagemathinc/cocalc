    ...
    CodeMirror.defineMode("makefile", function () {
        ...
        function tokenBase(stream, state) {
            ...
            if (sol) {
                ...
                // else, endif
                if (ch === "e" && (stream.match("lse") || stream.match("ndif"))) {
                    return "bracket";
                }
                // include
                if (ch === "i" && stream.match("nclude ")) {
                    return "string";
                }
                // -include
                if (ch === "-" && stream.match("include ")) {
                    return "string";
                }
                // vpath
                if (ch === "v" && stream.match("path ")) {
                    return "string";
                }
                // makros
                ...
                if ((stream.eat(":") || stream.eat("+")) && stream.peek("=")) {
                    return "variable-2";
                }
                if (ch === "$" && stream.eat("$") && stream.match(/^[\w]+/)) {
                    return "variable-2";
                }
                if (ch === "$" && stream.eat("(")) {
