...
const { walltime, defaults, required } = misc;

const log = getLogger("execute-code");

export const execute_code = aggregate((opts) => {
  opts = defaults(opts, {
    command: required,
    args: [],
    ...
    aggregate: undefined, // if given, aggregates multiple calls with same sequence number into one -- see @cocalc/util/aggregate; typically make this a timestamp for compiling code (e.g., latex).
    verbose: true,
    cb: undefined,
  });
  ...
  const env = misc.copy(process.env);
  ...
  async_series(
    ...
    (err) => {
      ...
      // log.debug(`(time: ${walltime() - start_time}): Done running '${opts.command} ${opts.args.join(' ')}'; resulted in stdout='${misc.trunc(stdout,512)}', stderr='${misc.trunc(stderr,512)}', exit_code=${exit_code}, err=${err}`)
      ...
      if (opts.verbose) {
        ...
        log.debug(
          `stdout='${misc.trunc(stdout, 512)}', stderr='${misc.trunc(
            stderr,
            512
          )}', exit_code=${exit_code}`
        );
      ...
      if (!opts.err_on_exit && ran_code) {
        ...
        opts.cb?.(false, { stdout, stderr, exit_code });
      } else {
