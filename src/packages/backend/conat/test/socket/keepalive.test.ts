...
describe("test a server with a short keepalive time", () => {
  ...
  it("creates a socket server with very short keepalive", async () => {
    cn1 = connect();
    server = cn1.socket.listen("keepalive.com", {
      keepAlive,
      keepAliveTimeout,
    });
    ...
    cn2 = connect();
    client = cn2.socket.connect("keepalive.com", {
      keepAlive: 10000,
  ...
  it("breaks the client side of the socket and observes the client automatically disconnects", async () => {
    client.sub.close();
