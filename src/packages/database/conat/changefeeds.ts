...
DEBUG_CONSOLE=yes DEBUG=cocalc:debug:database:nats:changefeeds
   ...
   require("@cocalc/database/conat/changefeeds").init()
...
const CHANGEFEED_INTEREST_PERIOD_MS = parseInt(
  process.env.COCALC_CHANGEFEED_INTEREST_PERIOD_MS ??
    `${CHANGEFEED_INTEREST_PERIOD_MS_USERS}`,
);
...
async function mainLoop() {
  ...
  try {
    for await (const mesg of api) {
      await handleRequest({ mesg, nc, cn });
    }
  } finally {
...
async function handleRequest({ mesg, nc, cn }) {
  let resp;
  try {
    numRequestsAtOnce += 1;
...
async function getResponse({ name, args, account_id, project_id, nc, cn }) {
  if (name == "userQuery") {
    ...
    if (!opts.changes) {
      ...
      return await createChangefeed(opts, nc, cn);
    }
  } else {
...
const createChangefeed = reuseInFlight(
  async (opts, nc, cn) => {
    ...
    if (changefeedInterest[hash]) {
      ...
      createNewChangefeed({ query, user, nc, opts, hash, cn });
    }
  },
  { createKey: (args) => jsonStableStringify(args[0])! },
);

const createNewChangefeed = async ({ query, user, nc, opts, hash, cn }) => {
