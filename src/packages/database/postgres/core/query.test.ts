...
describe("Query Engine - Group 6", () => {
  ...
  describe("USER FEEDBACK - Missing Coverage", () => {
    describe("Standby Protection - Write Guards", () => {
      beforeEach(() => {
        // Mock standby status
        (database as any).is_standby = false;
      });

      it("rejects opts.set on standby", (done) => {
        (database as any).is_standby = true;
      ...
      it("rejects opts.jsonb_set on standby", (done) => {
        (database as any).is_standby = true;

        database._query({
          table: "test_table",
          jsonb_set: { field: { key: "value" } },
          where: { id: 1 },
          params: [],
          cb: (err) => {
            expect(err).toBe("set queries against standby not allowed");
      ...
      it("rejects opts.jsonb_merge on standby", (done) => {
        (database as any).is_standby = true;

        database._query({
          table: "test_table",
          jsonb_merge: { field: { key: "value" } },
          where: { id: 1 },
          params: [],
          cb: (err) => {
            expect(err).toBe("set queries against standby not allowed");
      ...
      it("allows opts.values on standby (no guard for INSERT)", (done) => {
        (database as any).is_standby = true;

        // Create temp table first
        database._query({
          query:
            "CREATE TEMP TABLE IF NOT EXISTS standby_test (id SERIAL, value TEXT)",
          cb: (err) => {
            if (err) {
              (database as any).is_standby = false;
              return done(err);
            ...
            database._query({
              table: "standby_test",
              query: "INSERT INTO standby_test",
              values: { value: "test" },
              params: [],
              cb: (err) => {
                // Should succeed - no standby guard for values/INSERT
                expect(err).toBeFalsy();
                (database as any).is_standby = false;
                done();
              },
