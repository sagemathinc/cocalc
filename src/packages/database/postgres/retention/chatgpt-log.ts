...
const log = getLogger("database:retention:openai_chatgpt-log");

const MODEL = "openai_chatgpt_log";
...
period_counts AS (
  SELECT periods.period_start, COUNT(DISTINCT openai_chatgpt_log.account_id) AS count
  FROM periods
  LEFT JOIN openai_chatgpt_log ON openai_chatgpt_log.time >= periods.period_start AND openai_chatgpt_log.time < periods.period_end
  JOIN cohort ON openai_chatgpt_log.account_id = cohort.account_id
