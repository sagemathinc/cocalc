...
const log = getLogger("database:retention:openai_chatgpt_log-all");

const MODEL = "openai_chatgpt_log:all";

export default async function chatGPTLogAll({
  last_start_time,
  pool,
  start,
  stop,
...
ORDER BY periods.period_start`;
  ...
  const getSize = async (rows) => {
    ...
    return (
      await pool.query(
        "SELECT COUNT(DISTINCT(account_id)) as size FROM openai_chatgpt_log WHERE time >= $1::timestamp AND time < $2::timestamp",
