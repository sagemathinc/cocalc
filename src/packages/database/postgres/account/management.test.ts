...
describe("account management queries", () => {
  ...
  describe("touchAccount", () => {
    it("updates last_active timestamp", async () => {
      ...
      // Should have updated to within last 10 seconds, relative to DB time
      const { rows: diffRows } = await pool.query(
        "SELECT EXTRACT(EPOCH FROM (NOW() - last_active)) AS diff_s FROM accounts WHERE account_id = $1",
        [accountId],
      );
      const diffSeconds = parseFloat(diffRows[0].diff_s);
