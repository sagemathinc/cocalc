...
const { THEME_DEFAULT, COLOR_THEMES } = require("@cocalc/util/theme");

const isObject = (a) => !!a && a.constructor === Object;

// ── 1. Classic COLORS → $COL_* variables (backwards compatible) ─────

let colors_sass =
  "/* DO NOT UPDATE THIS FILE -- autogenerated via 'pnpm run update-color-scheme' based on @cocalc/util/theme.ts */\n\n";

colors_sass += "/* ── Static COLORS (legacy) ── */\n\n";
for (let c in COLORS) {
  // skip re-exported theme-colors symbols
  if (typeof COLORS[c] === "function") continue;
  if (Array.isArray(COLORS[c])) continue;
  const v = COLORS[c];
  if (isObject(v)) {
    for (let c2 in v) {
      const v2 = v[c2];
...
// ── 2. Default theme → $THEME_* variables ───────────────────────────

colors_sass += "\n/* ── Default Color Theme ── */\n\n";
for (const [key, value] of Object.entries(THEME_DEFAULT)) {
  if (typeof value !== "string") continue;
  // Convert camelCase to SCREAMING_SNAKE_CASE
  const sassKey = key.replace(/([A-Z])/g, "_$1").toUpperCase();
  colors_sass += `$THEME_${sassKey}: ${value}\n`;
}
