...
export function EditableMarkdown({
  ...
  const editor = useMemo(() => {
    ...
    ed.getSourceValue = (fragment?) => {
      return fragment ? slateToSource(fragment) : ed.getMarkdownValue();
    };
    ...
    ed.getMarkdownValue = () => {
      ...
      ed.markdownValue = slateToSource(ed.children, {
        cache: ed.syncCache,
      });
      return ed.markdownValue;
    };
  ...
  const [editorValue, setEditorValue] = useState<Descendant[]>(() =>
    sourceToSlate(value ?? "", false, editor.syncCache)
  );
      ...
      submitMentionsRef.current = () => {
        ...
        for (const [node, path] of Editor.nodes(editor, {
          ...
          mentions.push({
            ...
            description: slateToSource([parent]),
          });
        }
  ...
  const setEditorToValue = (value) => {
    ...
    editor.syncCache = {};
    // There is an assumption here that sourceToSlate produces
    ...
    // code we do nomalize the output of sourceToSlate, so
    // that assumption is definitely satisfied.
    const nextEditorValue = sourceToSlate(value, false, editor.syncCache);
