...
interface LineMarker {
  ...
  cellLabel: string; // "In[1]", etc.
  inputLineNum: number; // 1-based for input, 0 for output markers
  isFirstLineOfCell: boolean; // Show In[N] only on first line
  isOutputMarker: boolean;
  outLabel?: string; // "Out[1]" - shown on output marker line for all cells
...
function buildLineMarkers(mappings: CellMapping[]): Map<number, LineMarker> {
  ...
  mappings.forEach((mapping, cellIndex) => {
    // Use the actual execution count from the cell, fall back to index if not available
    const cellNum = mapping.execCount ?? cellIndex + 1;
    const outLabel = `Out[${cellNum}]`;
    const hasOutputs = (mapping.outputs?.length ?? 0) > 0;

    // Mark all input lines
    const { inputRange } = mapping;
    ...
    for (let line = inputRange.from; line < inputRange.to; line++) {
      ...
      markers.set(line, {
        ...
        // Show Out[N] label on the last input line for ALL cells (even without outputs)
        outLabel: isLastLineOfCell ? outLabel : undefined,
        hasOutputs,
        cellState: mapping.state,
      });
    ...
    markers.set(mapping.outputMarkerLine, {
      cellId: mapping.cellId,
      cellLabel: `Out[${cellNum}]`,
      inputLineNum: 0,
...
class CellLabelMarker extends GutterMarker {
  ...
  toDOM(): HTMLElement {
    ...
    // Add state indicator class for CSS styling
    ...
    // Top row: In[N] label and line number
    const topRow = document.createElement("div");
    topRow.className = "jupyter-cell-label-row";

    // Cell label (In[N]) - becomes a button when first line of cell
    if (this.isFirst) {
