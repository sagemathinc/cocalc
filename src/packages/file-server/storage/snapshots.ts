...
const logger = getLogger("file-server:storage:snapshots");
...
export async function updateRollingSnapshots({
  ...
  const written = await filesystem.writtenSinceLastSnapshot();
  ...
  const snapshots = Object.values(await filesystem.snapshots())
    .map((z) => z.snapshot_name)
    .filter((x) => DATE_REGEXP.test(x));
  snapshots.sort();
  ...
  await filesystem.createSnapshot(snapshot);
  // delete extra snapshots
  snapshots.push(snapshot);
  const toDelete = snapshotsToDelete({ counts, snapshots });
  for (const snapshot of toDelete) {
    await filesystem.destroySnapshot(snapshot);
  }
}
