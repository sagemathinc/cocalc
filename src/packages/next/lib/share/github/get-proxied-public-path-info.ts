...
import getPublicPathInfoGithub from "./get-public-path-info";
import getPublicPathInfoUrl from "./get-public-path-info-url";
import getPublicPathInfoGist from "./get-public-path-info-gist";

export default async function getProxiedPublicPathInfo(url: string) {
  if (url.startsWith("github/")) {
    return await getPublicPathInfoGithub(url);
  }
