...
export default async function getPublicPathInfoUrl(url: string, req) {
  const pool = getPool("short");

  // Get the database entry in public_paths that describes the URL
  const project_id = await getGithubProjectId();
  const id = sha1(project_id + url);
  ...
  if (rows.length == 0 || rows[0].project_id == null || rows[0].path == null) {
    ...
    await pool2.query(
      ...
      [id, project_id, url, `Document at ${url}`]
    );
    rows = (
      await pool2.query(
        `SELECT project_id, path, description, counter::INT,
  ...
  const projectTitle = `Document at ${url}`;
