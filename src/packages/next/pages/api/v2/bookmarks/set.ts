...
async function set(req: Request) {
  const account_id = await getAccountId(req);
  if (!account_id) {
    throw Error("must be signed in");
  }
  const { project_id, type, payload } = getParams(req);
  ...
  switch (type) {
    case STARRED: {
      ...
      await saveStarredFilesBookmarks({ project_id, starred: payload });
