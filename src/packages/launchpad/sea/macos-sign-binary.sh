...
# verify locally
codesign --verify --deep --strict --verbose=2 ./cocalc-launchpad


# this will fail:
spctl --assess --type execute --verbose=2 ./cocalc-launchpad
...
cp ./cocalc-launchpad payload/usr/local/bin/

export VERSION=0.1.0
# build unsigned pkg
pkgbuild --root payload \
  --identifier com.cocalc.launchpad.cli \
  --version $VERSION \
  --install-location / \
  unsigned.pkg
...
productsign --sign "Developer ID Installer: William STEIN (BVF94G2MB4)" \
  unsigned.pkg cocalc-launchpad-$VERSION.pkg

# notarize & staple the pkg
xcrun notarytool submit cocalc-launchpad-$VERSION.pkg --keychain-profile "notary-profile" --wait --progress

xcrun stapler staple cocalc-launchpad-$VERSION.pkg
