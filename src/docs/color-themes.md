...
```
    ...
    │ useResolvedColor│ │ tokens    │ │ on <body>       │
...
| `packages/frontend/app/render.tsx` | Injects `--cocalc-*` CSS custom properties on `<body>`, syncs `setDarkModeState()` |
...
`document.body`. This is the bridge between the React theme context and
...
All set on `document.body` by `render.tsx`:
