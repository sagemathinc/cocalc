...
RUN \
  apt-get update && \
  apt-get install -y texlive pandoc aspell poppler-utils net-tools wget git python python-pip make g++ sudo psmisc sagemath-upstream-binary haproxy nginx vim inetutils-ping lynx telnet git emacs subversion ssh m4
