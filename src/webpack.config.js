...
const COCALC_THEME = process.env.COCALC_THEME || "cocalc";
const KUCALC_MODE = COCALC_THEME == "kucalc";
...
const index_page = KUCALC_MODE ? "index-kucalc.pug" : "index-cocalc.pug";

for (let [fn_in, fn_out] of [[index_page, "index.html"]]) {
  staticPages.push(
...
for (let dp of glob.sync("webapp-lib/doc/*.pug")) {
  if (path.basename(dp)[0] === "_") continue;
  if (path.basename(dp).indexOf("software-") === 0 && COMP_ENV) continue;
  // outside KUCALC_MODE, only build the api page
  if (path.basename(dp).indexOf("api.") != 0 && !KUCALC_MODE) continue;
...
const setNODE_ENV = new webpack.DefinePlugin({
  ...
  CC_EDITION: JSON.stringify(KUCALC_MODE ? "kucalc" : "cocalc")
});
