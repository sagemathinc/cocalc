...
import json, logging, os, shutil, signal, string, StringIO, sys, tempfile, time, traceback
...
def _load_file():
    filename = namespace['sagews'].extra_data
    # TODO -- make secure!
    error = ''
    status = 'ok'
    try:
        ...
        status = 'error'
        error = str(msg)
    return json.dumps({'content':content, 'create':create, 'status':status, 'error':error})

# TODO: this is bad -- no need to even touch the namespace.
namespace['_load_file'] = _load_file

def _save_file():
    filename, content = namespace['sagews'].extra_data
    # TODO: make way more secure?
    open(filename,'w').write(content)

namespace['_save_file'] = _save_file

# This works and is nice, but incurs nasty overhead, which makes it not worth it.
## class ModuleWrapper(object):
##     def __init__(self, new_symbols, module, filename):
##         self.__dict__ = dict([(s, module.__dict__[s]) for s in new_symbols])
##         self._module = module
##         self._filename = filename
##     def __getattr__(self, attr):
##         return self._module.__dict__[attr]
##     def __repr__(self):
##         return '<sage module %s>'%self._filename
